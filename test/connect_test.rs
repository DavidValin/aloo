...
use aloo::connect::{
    cache_path, fresh_pq_hybrid_paths_in, prefill_connect_defaults, random_prefix, resolve_my_keypair, ConnectCache,
    ResolvedIdentity,
};
use aloo::ui::ui_connect_popup::{ConnectPopupState, MyKeySelection, MyKeyType};

fn temp_path(label: &str) -> PathBuf {
    std::env::temp_dir().join(format!("aloo-connect-test-{label}-{}-{}", std::process::id(), fastrand_seed()))
}
...
fn connect_cache_round_trips_through_save_and_load() {
    let path = temp_path("roundtrip");
    {
        ...
        cache.record("chat.example.com", 6667, "/home/me/.aloo/ab12.pub", "/home/me/.aloo/ab12.priv");
    ...
    }
    {
        ...
        assert_eq!(
            cache.most_recent(),
            Some(("chat.example.com", 6667, "/home/me/.aloo/ab12.pub", "/home/me/.aloo/ab12.priv"))
        );
    }
...
fn connect_cache_record_moves_an_existing_pair_to_most_recent() {
    ...
    assert_eq!(cache.most_recent(), Some(("bob.example.com", 2, "/b.pub", "/b.priv")));
    ...
    assert_eq!(cache.most_recent(), Some(("alice.example.com", 1, "/a2.pub", "/a2.priv")));
}
...
fn connect_cache_load_skips_malformed_lines() {
    ...
    assert_eq!(cache.most_recent(), Some(("carol.example.com", 9999, "/c.pub", "/c.priv")));
    std::fs::remove_file(&path).ok();
}
...
fn connect_cache_record_rejects_fields_containing_the_delimiter() {
    ...
    assert_eq!(cache.most_recent(), None, "a host containing a tab must never be stored");
    cache.record("host", 1, "/a\n.pub", "/a.priv");
    assert_eq!(cache.most_recent(), None, "a path containing a newline must never be stored");
}
...
fn fresh_pq_hybrid_paths_in_produces_a_pub_and_priv_pair_under_the_given_directory() {
    ...
    assert!(!file_pub.exists() && !file_priv.exists(), "neither file should be created yet");
}
...
fn fresh_pq_hybrid_paths_in_avoids_an_existing_colliding_prefix() {
    ...
    assert_ne!(second_pub, first_pub, "must not reuse a prefix whose files already exist");
...
fn prefill_connect_defaults_assigns_a_fresh_location_when_the_cache_is_empty() {
    ...
    assert_eq!(popup.host, original_host, "host is left alone when there's nothing to restore");
...
fn prefill_connect_defaults_restores_the_most_recent_cache_entry() {
    ...
    cache.record("chat.example.com", 6667, "/keys/ab12.pub", "/keys/ab12.priv");
    let mut popup = ConnectPopupState::new();
...
fn resolve_my_keypair_autogenerates_a_missing_pq_hybrid_bundle() {
    ...
    let sel = MyKeySelection::PqHybrid { file_pub: file_pub.clone(), file_priv: file_priv.clone() };
    let (identity, key_mode) = resolve_my_keypair(&sel).expect("should autogenerate and load successfully");

    assert_eq!(key_mode, aloo::proto::KeyMode::PqHybrid);
    assert!(matches!(identity, ResolvedIdentity::Pq { .. }));
    assert!(file_pub.is_file(), "the public bundle should now exist on disk");
    assert!(file_priv.is_file(), "the private bundle should now exist on disk");
