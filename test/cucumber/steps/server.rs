...
async fn handshake(stream: &mut TcpStream, name: &str, key_mode: KeyMode) -> UserId {
    ...
    assert!(
        matches!(hello, ServerMessage::Hello { auth: AuthKind::None, challenge: None }),
    ...
    assert!(matches!(result, ServerMessage::AuthResult { ok: true, .. }), "auth should succeed");
    ...
    let ServerMessage::IdentifyResult { ok: true, you: Some(you), .. } = identify else {
...
async fn both_joined_registry(w: &mut AlooWorld, a: String, b: String, channel: String) {
    ...
    let reg = w.registry_mut();
    reg.join_channel(ida, &channel, ChannelKind::Public).unwrap();
    reg.join_channel(idb, &channel, ChannelKind::Public).unwrap();
}
...
async fn join_channel(w: &mut AlooWorld, who: &str, channel: &str) {
    ...
    write_message(
        stream,
        &ClientMessage::JoinChannel { name: channel.into(), kind: ChannelKind::Public },
    )
    .await
    .unwrap();
}
...
async fn expect_message(w: &mut AlooWorld, who: &str) -> ServerMessage {
    let client = w.client_mut(who);
    let stream = client.stream.as_mut().expect("client has no socket");
    let msg: ServerMessage = tokio::time::timeout(
...
async fn duplicate_nickname(w: &mut AlooWorld, name: String) {
    ...
    write_message(&mut stream, &ClientMessage::Auth(AuthResponse::None)).await.unwrap();
    let auth: ServerMessage = read_message(&mut stream).await.unwrap().unwrap();
    assert!(matches!(auth, ServerMessage::AuthResult { ok: true, .. }), "auth itself should still pass");

    write_message(
        &mut stream,
        &ClientMessage::Identify { display_name: name, public_key_der: vec![], key_mode: KeyMode::Rsa },
    )
    .await
    .unwrap();
...
async fn offer_password(w: &mut AlooWorld, password: String) {
    ...
    let hello: ServerMessage = read_message(&mut stream).await.unwrap().unwrap();
    assert!(
        matches!(hello, ServerMessage::Hello { auth: AuthKind::Password, challenge: None }),
...
async fn join_confirmed(w: &mut AlooWorld, who: String) {
    ...
    assert!(matches!(msg, ServerMessage::Joined { .. }), "expected a Joined confirmation, got {msg:?}");
}
...
async fn told_that_joined(w: &mut AlooWorld, who: String, joiner: String) {
    ...
    match msg {
        ServerMessage::UserJoined { user, .. } => {
            ...
            assert_eq!(user.name, joiner, "the notification carries the joiner's nickname");
        }
...
async fn learns_then_joined(w: &mut AlooWorld, who: String, other: String) {
    ...
    match snapshot {
        ServerMessage::UserJoined { user, .. } => {
            assert_eq!(user.id, other_id, "the joiner should learn about the existing member");
        }
...
async fn nickname_refused(w: &mut AlooWorld, name: String) {
    let client = w.clients.get("impostor").expect("nobody tried a duplicate nickname");
    match client.received.first().expect("no identify result") {
        ServerMessage::IdentifyResult { ok: false, you: None, reason: Some(reason) } => {
            assert!(reason.contains(&name), "the reason {reason:?} should name the taken nickname");
        }
...
async fn connection_closed(w: &mut AlooWorld) {
    ...
    assert!(after.is_none(), "the server should close the connection after rejecting the nickname");
}
...
async fn default_channel(w: &mut AlooWorld, name: String) {
    ...
    assert_eq!(list.len(), 1, "a fresh server seeds exactly one channel, got {list:?}");
...
async fn private_unlisted(w: &mut AlooWorld, who: String, channel: String) {
    let id = w.id_of(&who);
    w.registry_mut().join_channel(id, &channel, ChannelKind::Private).unwrap();
    let list = w.registry_mut().channel_list();
    assert!(
...
async fn told_left(w: &mut AlooWorld, who: String, leaver: String, channel: String) {
    ...
    assert_eq!(w.emitted.len(), 1, "only the remaining member should be notified: {:?}", w.emitted);
    let out = &w.emitted[0];
    assert_eq!(out.to, who_id);
    match &out.message {
        ServerMessage::UserLeft { channel: got, user_id } => {
...
async fn rsa_auth(_w: &mut AlooWorld) {
    ...
    let challenge = cfg.make_challenge().expect("rsa auth must issue a challenge nonce");
    assert_eq!(challenge.len(), 32, "the documented nonce is 32 random bytes");

    let good = crypto::encrypt_chunked(&server_kp.public, &challenge).unwrap();
    assert!(cfg.verify(Some(&challenge), &AuthResponse::Rsa { blocks: good }), "the real key holder gets in");
...
async fn open_server_auth(_w: &mut AlooWorld) {
    ...
    assert!(cfg.make_challenge().is_none(), "no auth means no challenge to answer");
    assert!(cfg.verify(None, &AuthResponse::None), "an empty credential is the right one here");
    assert!(
