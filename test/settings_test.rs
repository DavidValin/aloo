use aloo::settings::{default_path, ServerAuth, Settings, DEFAULT_BIND, DEFAULT_GLOBAL_PTT_SHORTCUT, DEFAULT_PORT};
...
fn round_trips_password_auth_through_save_and_load() {
    let path = temp_settings_path();
    let saved = Settings { server_auth: ServerAuth::Password("hunter2".to_string()), ..Settings::default() };
    saved.save(&path).unwrap();

    let loaded = Settings::load_or_create(&path).unwrap();
    assert_eq!(loaded.server_auth, ServerAuth::Password("hunter2".to_string()));
...
fn round_trips_rsa_auth_through_save_and_load() {
    let path = temp_settings_path();
    let saved = Settings { server_auth: ServerAuth::Rsa(PathBuf::from("/tmp/server_key")), ..Settings::default() };
    saved.save(&path).unwrap();

    let loaded = Settings::load_or_create(&path).unwrap();
    assert_eq!(loaded.server_auth, ServerAuth::Rsa(PathBuf::from("/tmp/server_key")));
...
fn a_malformed_server_auth_type_with_no_matching_value_falls_back_to_none() {
    ...
    assert_eq!(settings.server_auth, ServerAuth::None, "a password type with no password line must not panic or half-apply");

    std::fs::write(&path, "server_auth_type=rsa\n").unwrap();
    let settings = Settings::load_or_create(&path).unwrap();
    assert_eq!(settings.server_auth, ServerAuth::None, "an rsa type with no keyfile line must not panic or half-apply");
...
fn load_skips_unparseable_or_unknown_lines_and_keeps_the_rest() {
    ...
    assert_eq!(settings.server_port, DEFAULT_PORT, "an unparseable server_port must keep the default");
    assert!(!settings.global_ptt_enabled);
