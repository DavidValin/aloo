use aloo::idstore::{default_path, IdCheck, IdStore};
use std::path::PathBuf;

fn temp_store_path() -> PathBuf {
    std::env::temp_dir().join(format!("aloo-idstore-test-{}-{}", std::process::id(), fastrand_seed()))
}
...
fn new_empty_starts_with_nothing_and_can_still_save() {
    ...
    assert_eq!(store.check_and_pin("alice", b"key-a"), IdCheck::New);
    store.save().expect("save should succeed even though the store started as new_empty rather than load");
    assert!(path.is_file());
...
fn same_nickname_different_key_is_a_mismatch() {
    ...
    assert_eq!(result, IdCheck::Mismatch { previous_public_key_der: b"key-a".to_vec() });
...
fn save_creates_missing_parent_directories() {
    let dir = std::env::temp_dir().join(format!("aloo-idstore-dir-test-{}-{}", std::process::id(), fastrand_seed()));
...
fn a_nickname_containing_a_tab_is_never_pinned() {
    ...
    assert_eq!(store.check_and_pin("alice\tevil", b"key-a"), IdCheck::New, "never actually pinned");
    assert_eq!(store.check_and_pin("alice\nevil", b"key-a"), IdCheck::New);
    assert_eq!(store.check_and_pin("alice\nevil", b"key-a"), IdCheck::New, "never actually pinned");
}
...
fn default_path_always_resolves_under_the_dot_aloo_home_directory() {
    ...
    assert_eq!(path.file_name(), Some(std::ffi::OsStr::new("ids_store")), "unexpected file name: {path:?}");
    assert_eq!(
...
fn corrupted_lines_in_an_existing_file_are_skipped_not_fatal() {
    ...
    assert_eq!(store.check_and_pin("carol", b"anything"), IdCheck::New, "the corrupt line should not have been pinned");
    std::fs::remove_file(&path).ok();
}
