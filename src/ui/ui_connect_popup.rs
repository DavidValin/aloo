...
pub enum MyKeySelection {
    None,
    Password(String),
    Rsa { file_pub: PathBuf, file_priv: PathBuf },
    ...
    RsaPerMessage { own_next_keys_path: PathBuf },
    ...
    PqHybrid { file_pub: PathBuf, file_priv: PathBuf },
}
...
impl ConnectPopupState {
    ...
    pub fn focus_order(&self) -> Vec<Field> {
        let mut v = vec![Field::Host, Field::Port, Field::Nickname, Field::IdStorePath, Field::ServerKeyType];
    ...
    fn activate_focused(&mut self) -> io::Result<Action> {
        match self.focus {
            ...
            Field::MyKeyValuePub if matches!(self.my_key.key_type, MyKeyType::Rsa | MyKeyType::PqHybrid) => {
                self.open_browser(FileBrowserTarget::MyKeyFilePub)
            }
            Field::MyKeyValuePriv if matches!(self.my_key.key_type, MyKeyType::Rsa | MyKeyType::PqHybrid) => {
    ...
    fn push_char_focused(&mut self, c: char) {
        match self.focus {
            ...
            Field::Nickname if !c.is_whitespace() && self.nickname.chars().count() < NICKNAME_MAX_LEN => {
    ...
    pub fn build_request(&self) -> Result<ConnectRequest, String> {
        ...
        let port: u16 = self.port.parse().map_err(|_| "port must be a number 1-65535".to_string())?;
        if port == 0 {
...
impl FileBrowserState {
    pub fn open(dir: PathBuf) -> io::Result<Self> {
        ...
        Ok(Self { current_dir: dir, entries, selected: 0, history_back: Vec::new(), history_forward: Vec::new() })
    }
    ...
    pub fn selected_path(&self) -> Option<PathBuf> {
        ...
        if entry.name == ".." {
            Some(self.current_dir.parent().map(|p| p.to_path_buf()).unwrap_or_else(|| self.current_dir.clone()))
        } else {
...
fn read_entries(dir: &std::path::Path) -> io::Result<Vec<DirEntryInfo>> {
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        if is_dir {
            ...
            files.push(DirEntryInfo { name, is_dir: false });
        }
    }
    ...
    if dir.parent().is_some() {
        entries.push(DirEntryInfo { name: "..".to_string(), is_dir: true });
    }
    entries.extend(dirs);
...
fn focus_border_style(focused: bool) -> Style {
    if focused { Style::default().fg(Color::Yellow) } else { Style::default() }
}
...
fn render_bordered_field(frame: &mut Frame, area: Rect, label: &str, value: &str, focused: bool) -> Rect {
    let block = Block::default().title(label).borders(Borders::ALL).border_style(focus_border_style(focused));
...
fn place_text_cursor(frame: &mut Frame, inner: Rect, offset: u16, value: &str) {
    let cursor_x = inner.x + (offset + value.chars().count() as u16).min(inner.width.saturating_sub(1));
...
pub fn render(frame: &mut Frame, state: &ConnectPopupState) {
    ...
    let host_inner = render_bordered_field(frame, chunks[0], "host", &state.host, state.focus == Field::Host);
    let port_inner = render_bordered_field(frame, chunks[1], "port", &state.port, state.focus == Field::Port);
    let nickname_inner =
        render_bordered_field(frame, chunks[2], "nickname", &state.nickname, state.focus == Field::Nickname);
    ...
    let server_key_value = match state.server_key.key_type {
        ...
        KeyType::Rsa => {
            if state.server_key.file.is_empty() { "<press Enter to browse>".into() } else { state.server_key.file.clone() }
        }
    };
    ...
    frame.render_widget(
        Paragraph::new(key_field_line("type", state.server_key.key_type.label(), state.focus == Field::ServerKeyType)),
        server_key_lines[0],
    );
    if state.server_key.key_type != KeyType::None {
        frame.render_widget(
            Paragraph::new(key_field_line("value", &server_key_value, state.focus == Field::ServerKeyValue)),
            server_key_lines[1],
        );
    }
    ...
    let my_key_lines = Layout::default()
        ...
        .split(my_key_inner);
    frame.render_widget(
        Paragraph::new(key_field_line("type", state.my_key.key_type.label(), state.focus == Field::MyKeyType)),
        my_key_lines[0],
    );
    match state.my_key.key_type {
        ...
        MyKeyType::Password => {
            ...
            frame.render_widget(
                Paragraph::new(key_field_line("value", &masked, state.focus == Field::MyKeyValuePub)),
                my_key_lines[1],
            );
        }
        MyKeyType::Rsa | MyKeyType::PqHybrid => {
            let pub_display =
                if state.my_key.file_pub.is_empty() { "<press Enter to browse>".to_string() } else { state.my_key.file_pub.clone() };
            ...
            frame.render_widget(
                Paragraph::new(key_field_line("file_pub", &pub_display, state.focus == Field::MyKeyValuePub)),
                my_key_lines[1],
            );
            frame.render_widget(
                Paragraph::new(key_field_line("file_priv", &priv_display, state.focus == Field::MyKeyValuePriv)),
                my_key_lines[2],
            );
        }
    }
    ...
    match state.focus {
        ...
        Field::ServerKeyValue if state.server_key.key_type == KeyType::Password => {
            place_text_cursor(frame, server_key_lines[1], VALUE_LABEL_LEN, &server_key_value)
        }
        ...
        Field::OwnNextKeysPath => {
            ...
            place_text_cursor(frame, my_key_lines[1], OWN_NEXT_KEYS_LABEL_LEN, &state.my_key.own_next_keys_path)
        }
        _ => {}
    }
    ...
    let hint = state.error.clone().unwrap_or_else(|| {
        "Tab: next field  Enter: select/connect  Esc: quit".to_string()
    });
    let hint_style = if state.error.is_some() { Style::default().fg(Color::Red) } else { Style::default().fg(Color::DarkGray) };
    frame.render_widget(Paragraph::new(hint).style(hint_style).alignment(Alignment::Left), chunks[10]);
...
fn render_connect_button(frame: &mut Frame, area: Rect, focused: bool) {
    ...
    let x = area.x + (area.width.saturating_sub(width)) / 2;
    let button_area = Rect { x, y: area.y, width, height: area.height };
    let inner = block.inner(button_area);
    frame.render_widget(block, button_area);
    frame.render_widget(Paragraph::new("Connect").alignment(Alignment::Center).style(text_style), inner);
}
