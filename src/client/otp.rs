...
pub(crate) async fn on_pad_commit(
    ...
    if already_this_pad || !session.otp_incoming_pads.contains_key(&from) {
        session
            .peer_link
    ...
    // same pad is recognised as one and a different pad is not.
    session.otp_store.mark_provisioned_with_pad(
        &contact_name,
