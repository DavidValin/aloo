...
| The "Direct Punches" popup's `OpenDirectPunches`/`SaveDirectPunchTargets` handlers actually reading/writing `~/.aloo/settings` and reconfiguring `PeerLinkManager` from a live session (AC-291's persist-and-reload half) | `docs/PROTOCOL.md` §7.1.5 | `session.rs::handle_ui_action`'s two new arms, the same "no test file of its own, needs a live `SessionState`" reason every other `handle_ui_action` row here gives; every mechanism they compose *is* covered directly - the popup produces the exact `SaveDirectPunchTargets` payload a save/delete should (AC-291's own UI-level tests), `Settings::update`'s merging write (`settings_test.rs`), and `PeerLinkManager::configure_direct_punch` rebuilding the scheduler from a target list (`direct_punch_test.rs`) - so what's untested is specifically the wiring that calls these in order from a live session. Verify manually: open Ctrl+S, add a target, confirm `~/.aloo/settings` gains a matching `direct_punch_to` line and the status line's count/next-try figures update within the next tick |
