...
| `features/` | Gherkin acceptance scenarios, one directory per story area (`channels/`, `connecting/`, `daemon/`, `diagnostics/`, `direct_punch/`, `encryption/`, `help/`, `identity/`, `messaging/`, `presence/`, `settings/`, `sharing/`, `voice/`). |
...
As of writing: 42 stories, 251 acceptance criteria, 209 technical
behaviours, 358 Gherkin scenarios, and 1119 Rust test functions (of which a
...
`encryption/`, `help/`, `identity/`, `messaging/`, `presence/`, `settings/`
(only what the Ctrl+S popup itself does - what each setting *means* lives
with the behaviour it changes), `voice/`). A
...
**One file deliberately has no feature-level tag**:
`features/encryption/message_encryption.feature`, which gathers every
peer-to-peer layering in one place and so spans three stories (US-027,
US-028, US-033). Inheriting a single `@US-` there would attribute each
story's scenarios to the other two, so every scenario carries its own
instead. Its scenarios also carry a tag naming the layering they exercise
— `@pqhybrid`, `@pqhybrid_otp`, `@direct_otp`, or `@otp_control` for the
ones about the pad layer's controls rather than its bytes — so
`cargo bdd -- -t "@direct_otp"` runs exactly one layering. The gate ignores
tags that are not requirement ids (`model.rs`'s `is_requirement_id`), so
these cost the model nothing.

**A description line must never begin with `@`.** Gherkin reads it as a
tag line, and the whole file is then skipped silently — `cargo bdd` stays
green while covering nothing from it. Watch the scenario count, not just
the exit code.
...
`identity.rs`, `messaging.rs`, `presence.rs`, `server.rs`, `settings.rs`,
`voice.rs`, plus `ui_common.rs`
...
| `forward_chunk`/`start_incoming_stream` actually replaying a `PendingChunkBuffer` entry through a live decrypt worker when a real chunk genuinely outraces a real `StreamStart` over a real socket (TB-267) | `docs/PROTOCOL.md` §7.3 | both take a live `SessionState` and a spawned decrypt-worker thread, the same live-socket reason every other `session.rs`-adjacent row in this table gives, and reproducing the actual race deterministically would need control over UDP arrival order this test harness does not have; `PendingChunkBuffer` itself - the buffering, replay-in-arrival-order, per-sender/per-stream isolation, and both bounds - is covered directly and had its regression tests proven to fail against a disabled buffer (`voice_stream_test.rs`'s `chunks_arriving_before_stream_start_are_replayed_in_arrival_order`, `pending_chunks_are_isolated_by_both_sender_and_stream_id`, `a_stream_start_that_never_arrives_is_swept_after_its_timeout`, `pending_chunks_stay_bounded_under_a_flood`), so what's untested is specifically the two call sites that wire it into a live stream. Verify manually: hold Space for well under a second, release, and confirm the recipient's log shows a real, audible clip rather than an empty one |
| Mixing of simultaneous incoming streams, and everything downstream of `mix_output` reaching a speaker | `docs/SPEC.md` #4 | needs a real output device. The *jitter buffer's* own behaviour is no longer part of this gap: `MixSource`/`mix_output`/`apply_mixer_cmd` are pure over a source map, so the latency management built on them is driven directly in `test/voice_test.rs` with no device at all (TB-273 through TB-277) - what remains untested here is only the device-facing half, the summing of several real streams into one real output stream |
| Echo ducking and echo-cancelling device selection actually breaking a real echo loop | `docs/SPEC.md` #4 'Echo' | needs a real microphone and speakers in one room, which is the entire mechanism. The decision logic either side of that is covered directly (`EchoDucker`'s gain trajectory under TB-278, `EchoProbe`'s speakers/headphones/mid-call-change/hysteresis behaviour against synthetic envelopes under TB-282, the device-name match under TB-281, the setting under AC-396); what cannot be tested here is whether 18dB of attenuation is in fact enough in a given room, whether a device that calls itself echo-cancelling cancels well, or - the one most worth a real room - whether `EchoProbe`'s thresholds separate an actual speaker echo from an actual quiet room, since the synthetic envelopes it is tested against are by construction exactly the two cases it is built to tell apart. **Not yet manually verified** - this needs two clients on one machine's speakers, checking that the far end stops hearing itself back and that talking over the far end still gets through |
...
| A deactivated account's next connect attempt showing its reason inline in red on the connect popup (`AccountDeactivatedError`, `docs/PROTOCOL.md` §5.5) | `docs/PROTOCOL.md` §5.5 | `handshake_as` is matched by downcast type directly inside `run_client_inner`'s loop (same function and reason as the row above) - and has no test of its own for *any* of its error branches today, not just this new one (`AuthRefusedError`'s identical sibling branch, in the same match, is equally untested). What it composes *is* covered directly: the wire contract that `AuthResult.deactivated` carries the reason (`a_superadmins_deactivate_blocks_the_next_login_with_the_reason`, `server_test.rs`) and that a correct password is required to learn it at all (`check_credentials_deactivated_still_requires_the_right_password`, `server_users_registry_test.rs`). Verify manually: `/deactivate` a nickname, then attempt to connect as it - the popup shows "this account has been deactivated: `<reason>`" in red and stays open |
| A plain `aloo --no-server` start skipping the connect popup, and either exiting immediately or bootstrapping straight into a serverless session | `docs/SPEC.md` "Running with no server at all"; `docs/PROTOCOL.md` §7.1.5 | same `run_client_inner` reason as the row above, one step earlier: `setup_surface` needs a real terminal before this branch is ever reached at all, so not even spawning the compiled binary (the `AC-094`-style seam two rows below) can reach it headlessly; the decision this branch is gated on (`Settings::has_direct_punch_configured`, fully tested) and the identity/`UserId` resolution it reuses (`resolve_my_keypair`, `client::p2p::direct_peer_id`) are both covered directly and unchanged by this branch. Verify manually: `aloo --no-server` with no `direct_punch_to` configured prints the explanation and exits at once with no popup shown; with one configured it goes straight to the connected UI, `serverless` true, reachable only by that peer |
...
| The durable send queue's *flush* half - `flush_outbox` actually re-sending a peer's queue over a link that has just become `Active`, in order, and the queue re-filling through the same `Undeliverable` path if that link drops mid-drain (AC-410) | `docs/SPEC.md` Functionality #34 | it hangs off `session.rs`'s `LinkStatusChanged`/`Active` arm, which needs a genuinely punched link between two live sessions - the same live-socket reason every other `session.rs` row here gives. Both halves either side of it *are* covered directly: the store's ordering, bounds, persistence and refusals (`outbox_test.rs`), and the transport-to-session wiring that fills it, including that the transport keeps no second copy (`session_paths_test.rs`). Verify manually with two clients: send while the second is closed, confirm `~/.aloo/outbox/<nick>.q` grows, then start it and confirm the messages arrive in the order they were sent |
| The Ctrl+S settings popup's `OpenSettings`/`SaveSettings`/`SaveDirectPunchTargets` handlers actually reading/writing `~/.aloo/settings`, mirroring the live values onto `SessionState`/`UiState`, and reconfiguring `PeerLinkManager` from a live session (AC-291, AC-397, AC-398, AC-399's persist-and-reload half) | `docs/PROTOCOL.md` §7.1.5; `docs/SPEC.md` Functionality #23 | `session.rs::handle_ui_action`'s two new arms, the same "no test file of its own, needs a live `SessionState`" reason every other `handle_ui_action` row here gives; every mechanism they compose *is* covered directly - the popup produces the exact `SaveDirectPunchTargets`/`SaveSettings` payload a change should (`ui_direct_punch_popup_test.rs`, `ui_settings_popup_test.rs`), `SettingsDraft::apply_to` turning that payload into settings without ever writing a value that is not one (`ui_settings_popup_test.rs`), `Settings::update`'s merging write and every new key's round trip (`settings_test.rs`), and `PeerLinkManager::configure_direct_punch` rebuilding the scheduler from a target list (`direct_punch_test.rs`) - so what's untested is specifically the wiring that calls these in order from a live session. Verify manually: open Ctrl+S, add a target on the Direct Punch tab, confirm `~/.aloo/settings` gains a matching `direct_punch_to` line and the status line's count/next-try figures update within the next tick; flip `roger_beep` off on the General tab and confirm the next voice message ends silently without a restart; flip `global_ptt_enabled` off and confirm the OS-level shortcut stops recording immediately |
| `client::otp_mail::refresh_unread_mail_count`'s call sites (mail arrival, `handle_read`, `handle_delete`, session start) actually keeping the header's unread count current in a live session (AC-293's refresh half) | `docs/PROTOCOL.md` §17 | `otp_mail.rs`'s arrival/read/delete handlers all take a live `SessionState`/`ControlSink`, the same reason every other `session.rs`-adjacent row here gives; every mechanism they compose *is* covered directly - the count itself (`OtpMailStore::unread_received_count`/`mark_read`, `otp_mail_store_test.rs`) and the header's rendering of it (`ui_channel_test.rs`'s `header_shows_the_unread_otp_mail_count_right_before_conn_quality`/`the_unread_otp_mail_indicator_blinks`) - so what's untested is specifically the four call sites that keep `UiState` in sync with the store as mail arrives, is read, or is deleted. Verify manually: receive an OTP mail and confirm the header shows "1 unread OTP Mails" blinking; open and read it and confirm the indicator disappears |
| `RejectIdentity`'s network-facing side effects, and `AcceptIdentity`'s queued-send flush specifically (the `id_store` persist half of `AcceptIdentity` - and, with it, `check_identity`/`finalize_identity_pin`'s full live per-device algorithm, device-blind provisional pin at `UserJoined` through to an additive, device-scoped review - is now covered for real, end to end over a real server and two real `run_daemon_session`s, `features/identity/identity_pinning_live.feature`'s two scenarios, `test/cucumber/steps/identity_live.rs`) | `docs/PROTOCOL.md` §12.4, §12.7 | lives in `session.rs::handle_ui_action`, which - like the rest of `session.rs` - has no test file of its own for what remains (needs a live socket); the pure `UiState` bookkeeping half (`resolve_identity_accept`/`resolve_identity_reject`) is covered directly, and every `IdStore` primitive the live algorithm is built from is covered in full at the unit level (`idstore_test.rs`) |
...
| File/voice content actually flowing end-to-end under an active OTP session - offer (its own pad spend, AC-148), accept/auto-accept, a second independent content-phase pad spend, chunked transport, whole-content decrypt, and each phase's own resulting ack (AC-146) - including, once that decrypt lands, whether the clip actually reaches the mixer (`suppress_playback_from`/`is_viewing_dm`, then `session.mixer_tx`) exactly when a live `pq_hybrid` stream would have (AC-357) | `docs/PROTOCOL.md` §16.2 | `client::otp::send_file_offer`/`on_file_offer`/`start_outgoing_file_content`/`finish_incoming_file`/`send_voice_offer`/`on_voice_offer` and their `session.rs` call sites (`accept_file_offer`, `handle_file_event`, `handle_p2p_event`'s `FileAccepted`/`FileRejected`/`OtpFileContentSeq` arms) all take a live `SessionState`/`ControlSink`/spawned worker threads and need a real socket to exercise meaningfully, the same reason the three rows above do; the mechanism each piece relies on *is* covered directly - small-blob `otp --encrypt`/`--decrypt` round-tripping (`a_message_encrypted_by_alice_decrypts_to_the_same_bytes_for_bob`, the same path the offer rides), whole-file `otp --encrypt`/`--decrypt` round-tripping without buffering (`encrypt_file_and_decrypt_file_round_trip_without_buffering_in_memory`), failing closed on a second encrypt without proof of delivery (`decrypt_file_without_assume_delivered_twice_fails_closed_on_the_second_call`), and the autoplay decision `finish_incoming_file` hands off (`UiState::on_direct_voice_message`'s `listened` behavior, covered directly against `UiState` with no session needed at all - `on_direct_voice_message_sets_listened_from_the_caller`, `an_unlistened_otp_voice_row_shows_a_red_not_listened_marker`, `a_listened_otp_voice_row_shows_no_marker` in `ui_direct_message_test.rs`, and the three OTP scenarios in `features/voice/voice_listened_marker.feature`) - so what's untested is specifically the wiring that calls these at the right moments, and in the right order (offer's own ack must close its slot before the content phase may open a second one), plus whether `finish_incoming_file` itself computes `suppress_playback` correctly and actually pushes to `session.mixer_tx` rather than just deciding `listened` right; verified manually with two clients (large file over an OTP session arrives with the pad prefix and is byte-identical to the source once downloaded; holding Space to an OTP-active peer records silently with no live playback on the peer's end, and the clip appears as one finished, playable entry on both sides after release; a voice message sent under OTP to a peer with that DM open plays itself the moment it finishes decrypting, the same as a `pq_hybrid` one, and stays silent - marked "not listened" - when sent to a peer looking at something else) |
| The nonce-bound acknowledgement crossing a *real UDP socket*, and a recovery-resend of one firing on a genuine reconnect (AC-250, AC-251) | `docs/PROTOCOL.md` §16.2, `docs/SPEC.md` "Proving an acknowledgement" | the wiring itself is no longer a gap - `otp_ack_wiring_test.rs` drives two real `SessionState`s holding the two halves of one real pad through `otp::send_or_queue` -> `otp::on_message` -> `otp::on_delivery_ack`, asserting the gate and the row's arrow for a proved ack, an unprovable one, a mismatched sequence, a queued message released only by the proved one, and both directions gating independently; what those sessions do not do is punch each other, so the payload is handed across by the test rather than by the transport, and `recover_and_resend` still needs a live `LinkStatusChanged` to fire (the same reason as the row below). Verify manually with two clients by exchanging several messages each way over one `/otp` session - neither side should ever stall behind its own gate, and every arrow should reach green |
| A recovery-resend actually firing on `LinkStatusChanged`'s `Active` transition and reaching the right peer (AC-147) | `docs/PROTOCOL.md` §16.4 | `client::otp::recover_and_resend`'s `session.rs` call site (`handle_p2p_event`'s `LinkStatusChanged` arm) needs a live link transition to exercise - same live-socket reason as the row above. The recovery pass *itself* is no longer a gap: `a_lost_file_offer_and_a_lost_voice_offer_each_recover_as_their_own_kind` drives `recover_and_resend` over two real `SessionState`s holding the two halves of one real pad, asserting that a dropped file offer and a dropped voice offer each come back framed and addressed as their own kind (`recover_and_resend_offer`'s `OfferKind`), under the original stream and slot, with no further pad spent, and that the peer's decoder accepts the replay and acks that same slot. The rest of the mechanism it relies on *is* covered directly - `otp --recover-last` replaying the exact last-sent ciphertext without spending key, both in-memory and file-to-file (`recover_last_sent_replays_without_consuming_key`, `recover_last_file_replays_the_last_sent_ciphertext_without_consuming_key`), the pending-send descriptor round-tripping through the store file (`pending_content_round_trips_through_save_and_load`), and - the property that makes any of this safe to attempt at all - a resend of an already-accepted sequence being rejected before `otp --decrypt` ever runs on it again (`is_next_expected_rejects_a_resend_of_an_already_accepted_sequence`); verify manually by restarting one client mid-conversation, right after a send, before its ack could arrive, and confirming the message resends automatically on reconnect using the recovered ciphertext rather than a fresh encode |
...
| A live call's network/audio orchestration actually running end to end - `voice_call.rs`'s `begin_own_call`/`add_participant`/`on_call_accept`/`on_call_roster`/`end_own_call`/`spawn_call_audio_worker`/`spawn_call_decrypt_worker`, the roster-convergence rules (`docs/PROTOCOL.md` §7.7) actually reaching a full mesh across three or more real participants regardless of join order, `host_set_muted`/`on_call_mute`/`invite_to_call` actually reaching every participant over the wire, `on_call_end`'s host branch actually ending the call on every other client, the two voice-meter feeds (`voice::level_from_pcm` off the capture and decrypt workers) actually reaching `UiState::set_call_level` through `SessionState::call_level_tx`, `channel.rs`/`direct_message.rs`'s `handle_start_call` actually excluding/refusing an OTP-active peer, and `on_call_invite`'s busy check actually auto-declining a second invite (AC-172-ish territory, deliberately never minted as a formal AC - see below) | `docs/PROTOCOL.md` §7.7, `docs/SPEC.md` Functionality #14 | these need both a live `SessionState`/`ControlSink` (the same reason every `session.rs` row above gives) *and* a real microphone/speaker (the same reason the jitter-buffer/mixing row near the top of this table gives) - two of this table's recurring gaps at once. Every mechanism they compose *is* covered directly: the wire round trip of all six signaling messages (`live_call_message_family_roundtrips`, TB-199; `host_mute_and_roster_messages_roundtrip`, TB-207), the popup/queue/held-and-revealed mechanics, the permanent indicator, the whole call modal (roster labels, live duration, voice meters, minimize-to-tab, END CALL, the host's `m`/`i`) and the `/call` confirmation (`test/ui_test.rs`'s call tests and `features/voice/live_call.feature`, AC-167-AC-171 and AC-175-AC-183), the meter arithmetic itself (`level_from_pcm_reads_rms_loudness_clamped_to_a_hundred`, TB-208), the hold-and-replay of a key setup that outran its participant (`test/voice_call_test.rs`, TB-209), the identical hold-and-replay of audio chunks that outran their participant (`ActiveCall::pending_chunks` reuses `voice_stream::PendingChunkBuffer` verbatim, proven under TB-267/TB-268 in `voice_stream_test.rs` - nothing call-specific to cover there either, since the type is unchanged), and the per-chunk sealing the call's audio workers reuse unchanged from push-to-talk (`voice_stream_test.rs`, `voice_test.rs` - nothing new to cover there, since none of it was touched). What's genuinely untested is specifically the session-level wiring that calls these in order, and the roster-convergence rules' real-world correctness across more than two live participants - both needing the same kind of manual multi-client verification (two, then three, same-LAN clients) the other `session.rs` rows above call for |

| `/endotp` actually running end to end in a live session - `handle_end_otp_command`'s local-pause-then-notify sequence, `on_end_session`/`on_end_session_ack` applying an arrival and clearing the retry, `resend_pending_end_notices` actually firing on `LinkStatusChanged`'s `Active` transition, and `handle_server_message`'s `UserJoined` arm actually re-marking a reconnected, already-provisioned and unpaused peer's session active (AC-192, AC-193, AC-194; the paused case, and the same re-derivation from the `DeviceIdAnnounce` arm, are covered directly - AC-443) | `docs/PROTOCOL.md` §16.6 | `client::otp`'s handlers and their `session.rs` call sites (`handle_ui_action`'s `EndOtpSession` arm, `handle_p2p_event`'s `LinkStatusChanged` arm, `handle_server_message`'s `UserJoined` arm) all take a live `SessionState`/`ControlSink`, the same live-socket reason every `session.rs` row above gives; every mechanism they compose *is* covered directly - the pure end/refuse decision (`decide_end_otp_refuses_when_nothing_is_provisioned`, `decide_end_otp_refuses_while_a_mail_is_in_flight`, `decide_end_otp_allows_ending_with_a_plain_pending_send`, `decide_end_otp_allows_ending_a_quiescent_session`), the per-contact pause (pad and sequence counters kept, only the pending gates cleared) and the durable notice's persistence (`pause_session_clears_pending_state_but_keeps_the_pad_and_owes_a_notice`, `pause_after_peer_ended_clears_pending_state_but_owes_no_notice_of_its_own`, `pausing_one_contacts_session_does_not_touch_another_contacts_state`, `a_pending_end_notice_survives_save_and_load`, `clear_end_notice_reports_whether_anything_was_owed`, `pending_end_notices_yields_only_contacts_still_owed_one`), the wire payload round-tripping (`end_session_payload_round_trips_through_the_wire_encoding`), and the UI-facing surface driving it (`endotp_in_an_open_dm_room_produces_end_otp_session_for_that_peer`, `endotp_outside_any_open_dm_room_is_a_no_op`, `endotp_can_still_be_typed_and_submitted_while_the_open_dm_peer_is_offline`, `clear_otp_active_reverses_mark_otp_active_and_drops_the_key_status_snapshot`, `a_disconnect_alone_does_not_end_an_active_otp_session`, `message_encryption.feature`) - so what's untested is specifically the wiring that calls these in order, and in particular the retry actually resuming after a real reconnect; verified manually with two clients (end alice's session with an online bob - both sides show "ended" immediately, and a fresh `/otp` between them resumes the same pad rather than generating a new one; end it while bob is offline, then reconnect him - he's told on reconnect; disconnect/reconnect bob mid-session without ending it - the pad marker and header are still there once he's back) |

| A genuine `PqWrapped` OTP match in the unknown-nickname reconciliation scan actually running end to end - `session::scan_pinned_keys_for_match`'s `OtpMessage` branch calling the real `otp::recover_padded_otp_bytes` (the outer `pq_hybrid` seal check) against a candidate that decodes, finding it, then `otp::finish_opening_otp_envelope` actually spending the pad exactly once, and `handle_ui_action`'s `ConfirmUnknownPeerKey` arm finishing registration from it via `otp::apply_otp_message` (AC-276, AC-277) | `docs/PROTOCOL.md` §7.1.5 | needs a live punched link plus a real, already-provisioned OTP pad layered on a `pq_hybrid` identity pinned under a *different* nickname than the one punching in - `test/daemon_session_test.rs`'s `an_unknown_but_matching_nickname_is_offered_and_then_pinned` proves the identical shared machinery (the popup, the scan dispatch, the ban wiring, the confirm-and-pin) end to end for the plain `ChannelPresence` case instead, since that needs no external `otp` binary or pad file; every OTP-specific piece this branch depends on *is* covered directly elsewhere - `decrypt_own_blinded_envelope`'s side-effect-free-on-failure property (the same signature-before-replay-accept ordering `decrypt_own_envelope` has, read directly in `session.rs`), `open_otp_envelope`'s ordinary-flow behaviour (`otp_ack_wiring_test.rs`'s `PqWrapped` scenarios, unchanged by the `recover_padded_otp_bytes`/`finish_opening_otp_envelope` split), and `contact_name_for`'s symmetry - so what's untested is specifically this one branch's wiring, not the cryptography or the shared UI/session/OTP machinery it reuses; verified manually by pinning the same `pq_hybrid`-plus-pad identity under two nicknames and confirming the second one's first OTP message triggers the offer and, once confirmed, the existing pad session. Reconciling a pad-*only* (`Direct`-framing) sender is not supported at all, by design - see `docs/PROTOCOL.md` §7.1.5's own explanation of why. |
| A shared-folder send that bails out *after* its `SharedFileTag` has gone - the pad gate busy (`otp::send_file_offer`'s `unacked` check), or a key that will not seal - and the stream id that tag named being burned so no later send inherits the requester's expectation (AC-455) | `docs/PROTOCOL.md` §7.8 | the rotating-key case, the other way a send can be refused, is not reachable here at all: `shared::pump_shared_sends` waits on `RemoteKeys::can_use` rather than starting a send it cannot finish, which `a_folder_download_is_offered_one_file_at_a_time` covers. What remains is the OTP branch, and reaching it needs a provisioned pad contact with an unacknowledged spend already outstanding - a `peer_device_ids` entry, a keychain name derived from both sides' fingerprints, and an `otp_store` primed with a pending send - to exercise three lines that burn one integer. The property those lines protect *is* covered from the other side: `an_unrequested_tag_still_gets_the_popup` pins that an expectation only ever matches a tag this side asked for, and `a_tagged_offer_is_accepted_without_a_popup` that the expectation is consumed by the one offer it named |
| A banned IP's `DirectPing` actually being dropped by a real `on_direct_ping` call reached through a live loopback socket, rather than through `IpBanList`'s own unit tests (AC-281) | `docs/PROTOCOL.md` §7.1.5 (No-IP updates section's IP ban list) | the check itself is two lines at the very top of `on_direct_ping` (`direct.ip_bans.is_banned(addr.ip())`), trivially correct by reading; a live test would need `ALOO_HOME` redirected to a scratch ban file, and `test/direct_punch_test.rs`'s tests share one process-wide environment with no serialisation between them (unlike `daemon_session_test.rs`'s single serialised `shared_home`), so seeding a ban for the loopback address every one of them punches over would corrupt every other test in the file rather than just this one; the ban logic itself is fully covered directly (`test/ip_ban_test.rs`), and the two-line call site is exercised implicitly by every direct-punch test that never gets banned and keeps punching normally |
