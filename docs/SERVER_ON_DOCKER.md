...
| `ALOO_FEDERATION_ID` / `ALOO_FEDERATION_PORT` / `ALOO_FEDERATION_ADVERTISE_ADDR` / `ALOO_FEDERATION_IDENTITY` | `server_federation_id` / `_port` / `_advertise_addr` / `_identity` settings | This server's own federation identity - a PQ-hybrid keybundle prefix (no TLS/certificates), generated on first start if missing; put it in a mounted volume so it survives a container restart. Note the ordering problem: a peer cannot be *configured* until you hold its `.pub`, and the container only generates its own on first start - so either start each server once with no peers configured, or pre-generate with `aloo --keygen-pq-hybrid <prefix>` and mount the result. `ALOO_FEDERATION_ID` must not be empty or contain a tab, newline or comma; the server refuses to start otherwise. |
| `ALOO_FEDERATION_CLIENT_ADDR` | `server_federation_client_addr` setting | The `host:port` an ordinary *client* should be told to connect to for this server, used when a peer redirects a user who logged in on the wrong one. This is the client-facing port (7878 by default), **not** the federation port - and it is announced to peers over the link, so set it here rather than in every peer's config. |
