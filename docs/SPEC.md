...
- **email** and the **Register** button — always shown and always focusable, regardless of `registration_available` (captured once from the local `server_allow_registration` setting when the popup opens). A server that takes no registrations is never hidden from — pressing **Register** on one is refused immediately, in red ("this server does not accept registrations right now"), the same inline-error path any other invalid submission uses, with no network round trip. **email** is its own titled, bordered box, read only by Register - Connect never looks at it. Refuses whitespace while typing; must look like an email address to register with.
