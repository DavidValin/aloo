...
8. **Identity pinning (`id_store`): deciding whether to trust a nickname that reconnects under a different key.** Full model in `docs/PROTOCOL.md` §12; from the user's point of view:
   ...
   - If a nickname with a pinned key reconnects with a **different** key — a byte change (`docs/PROTOCOL.md` §12.4) — the **identity review popup** (see "Connected UI" above) opens automatically, naming the user and a short fingerprint of both the old and the new key. This can mean the person genuinely regenerated their key, or that someone else is now using that nickname — the app doesn't decide which; it puts the decision to the user via **Accept** or **Reject**, rather than guessing.
...
| Per-contact OTP state, ack gate (§16.2) | `client/otp_store.rs` `OtpStore`, `OtpContactState`, `clear_pending`; `client/otp.rs` `OtpOutQueue`, `send_or_queue`, `on_delivery_ack`, `release_pending_and_drain` |
...
| `OtpEnvelope`/`OtpFileOffer`/`OtpVoiceOffer`/`OtpDeliveryAck` (§16) | `p2p_proto.rs` `P2pPayload` |
| File content under the pad, whole-file encrypt/decrypt (§16.2) | `client/otp_cli.rs` `encrypt_file`, `decrypt_file`, `encrypt_file_retrying`, `decrypt_file_retrying`, `FileCliOutcome`; `client/otp.rs` `send_file_offer`, `prepare_outgoing_file_content`, `on_file_offer`, `finish_incoming_file`, `temp_content_path`, `secure_remove_file`; `client/file_transfer.rs` `OwnFileTarget.otp`, `OtpIncomingFileReceive`, `OtpIncomingKind`; `client/session.rs` `SessionState.otp_send_temp_files`/`otp_incoming_file_receives`, `accept_file_offer`, `handle_file_event`, `handle_p2p_event`'s `FileAccepted`/`FileRejected` arms |
