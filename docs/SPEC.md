...
17. **Direct punching: keeping a link to someone with no server involved.** Full model in `docs/PROTOCOL.md` §7.1.5; from the user's point of view:
    ...
    - **A line may name several ports, all tried at once.** `direct_punch_to=bob,bobpublic.com:[18000,19000,21000],every_1m` probes all three on every slot. Worth doing because most routers rewrite the port your client sends from, so a single agreed number often arrives nowhere — which one survives is the router's choice, and naming a few only needs one to get through. The moment one answers, that address is the only one used from then on; losing the link puts them all back in play.
    - **You listen on several ports too.** `direct_punch_port=18000,19000,21000` binds all three. This is the half that makes a shared list work: someone can only reach you on a port you *send from*, never one you merely aim at, so probes are paired — the socket bound to 18000 probes the peer's 18000 — and two clients running the same list are reachable on all of it. A local port already in use is reported and skipped rather than stopping the others; only all of them failing falls back to an ephemeral socket.
