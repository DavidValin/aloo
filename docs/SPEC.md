...
Joining a channel homed on a different federated server works by proxy: the request goes to the owning server, which is the only one that ever checks the password/ban/allowlist, and the requesting server mirrors a successful grant locally so its own client sees an ordinary `Joined`. OTP mail addressed to a nickname registered on a different federated server is forwarded there and delivered exactly as it would be locally, with delivery receipts relayed back the same way - the uploading server keeps its own durable copy until the peer confirms receipt, so a briefly-unreachable link delays delivery rather than losing it. Neither feature lets two clients connected to *different* federated servers actually exchange a live message in a shared channel yet, since the direct peer-to-peer link candidate exchange (§7.1) is not itself federated - see `docs/PROTOCOL.md` §18.5.

Deleting a channel or removing an account gossips that removal too, so a name a server no longer has stops being permanently blocked federation-wide - a peer mirroring a deleted channel force-deletes its own copy and tells its local members, the same as a local deletion would (`docs/PROTOCOL.md` §18.8). Every federation event a server takes part in - a peer link coming up or down, directory gossip sent or received, a channel or nickname's removal, a channel join proxied to or from a peer, and OTP mail forwarded, received, or confirmed delivered - prints one line to the server's own console (`Federated server <peer> - <EVENT>`), always just an event name and which peer it concerns, never a nickname, channel name, or any message content.
