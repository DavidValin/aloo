...
13. **OTP mail: write someone a mail that waits, one-time-pad encrypted, on the server until they connect.** Full model in `docs/PROTOCOL.md` §17; from the user's point of view:
    - **`/mail`** in the compose bar opens a **full-screen compose view** — a command rather than a key chord, since the natural chord (Ctrl+M) and Enter are the same byte on terminals without the kitty keyboard protocol: From (fixed to your own nickname), To, a **Device** selector, Subtext (the subject line), a multi-line Content box, and an attachments pane. Tab/Shift+Tab cycle the fields; they can be filled independently, in any order. Esc discards the draft and returns to whatever view was underneath.
