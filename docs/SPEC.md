...
src/client/outbox.rs               <-- the durable send queue: what a message for someone unreachable waits in (queue_send_messages), Functionality #34
src/client/otp_outbox.rs            <-- the pad session's own durable send queue: sealed messages, one per ack, in order (queue_send_messages), Functionality #34
...
src/client/tui/transfers_popup.rs  <-- the Ctrl+D transfers popup: every transfer, both directions, cancel/resume/clear (likewise), Functionality #35
src/client/tui/shared_browser.rs  <-- the Browse shared files popup: a peer's shares, listings asked of them, d to download (likewise), Functionality #35
src/client/tui/share_popup.rs      <-- the File Sharing tab's shared-folder list and its add/edit form (likewise), Functionality #35
src/client/tui/otp_mail.rs        <-- /mail //mailbox OTP mail surface: compose view, mailbox popup, reader (likewise), Functionality #13
src/client/tui/settings_popup.rs   <-- the Ctrl+S settings surface: the four tabs, their fields, and the draft each change saves (likewise), Functionality #23
```
...
- `test/cucumber/main.rs` (`harness = false`) — a behaviour-driven acceptance layer running the Gherkin scenarios under `features/` via cucumber-rs, grouped by capability (`channels/`, `connecting/`, `encryption/`, `help/`, `identity/`, `messaging/`, `presence/`, `settings/`, `voice/`) rather than by source file.
...
4. **Send a voice message** by holding Space (while focus is not on the compose bar - Space there just types a literal space) and releasing it to stop, up to `voice::MAX_RECORDING_SECS` (4 minutes) long. Voice is streamed live, not recorded-then-sent: while Space is held, captured audio is chunked (`voice::CHUNK_INTERVAL`, 15ms) and sent to the network as it's captured, and the receiving side plays each chunk as it arrives, rather than waiting for the whole message. While recording, the compose bar is taken over: its border turns red, a blinking red ⏺ "recording..." indicator sits at the start of the bar in the same place every time, and whatever was typed there is hidden along with the cursor until the recording ends - kept, not discarded, and shown again on release.
   ...
   - **Hold-to-talk by touch.** A third trigger, for a tablet with no keyboard: the left button - a finger on a touch screen, or a mouse button - held anywhere on the screen for `TOUCH_HOLD_THRESHOLD` (a quarter of a second) starts the same live stream to the same target, and its release stops it. A tap (down and up inside the threshold) stays the ordinary click of item 31, and only a press still held past it records. The click is honored on the press as before, so holding on the input bar both focuses it and records. Like the global shortcut, a touch recording is never subject to Space's idle-silence guess, and no trigger can stop a recording another one started. A press while a touch recording is already running stops it instead of starting another - that is what a stuck recording looks like on a terminal that kept the release for its own long-press text selection, and a tap is the way out. Nothing starts while a popup or any other overlay is in front of the view. `touch_ptt_enabled` in `~/.aloo/settings` (on by default; also on the `Ctrl+S` General tab) turns the gesture off.
   - **What the terminal has to provide, and what happens when it does not.** Every trigger is push-to-talk: the microphone is open exactly while something is held - a key, the OS shortcut, a finger - and never otherwise; no tap, no toggle, no silence detection ever opens or closes it. A held finger therefore needs a terminal that forwards the press when it lands and the release when it lifts. Most touch terminals do not: Termux, and the terminal apps generally found on a tablet, send a finger as a press and release *together*, the moment it lifts, and keep a long press for their own text selection - so a held finger never reaches aloo there, and holding can do nothing. aloo recognises that back-to-back pair (a press and release within `TAP_PAIR_WINDOW` of each other - a mouse click, or a finger on a terminal that reports presses for real, keeps the button down several times longer) and, the first time it sees one while `touch_ptt_enabled` is on and nothing is recording, says so once in the status line: `hold-to-talk needs a terminal that forwards a held finger - this one reports taps only (use Space, the global shortcut, or another terminal app)` (`TAP_ONLY_TERMINAL_NOTICE`). The tap itself stays the ordinary click of item 31, and nothing records.
...
8. **Identity pinning (`id_store`): deciding whether to trust a nickname that reconnects under a different key, and from a different device.** Full model in `docs/PROTOCOL.md` §12; from the user's point of view:
   ...
   - **Exporting this client's own identity card** — the live-session equivalent of `aloo --export-identity-card`, no arguments needed since a live session already has its own keybundle and nickname loaded. Reachable two ways: the `x` shortcut from anywhere in the list, or an **Export identity card (own pqhybrid key)** button that is always the list's own last entry, one position past the last real contact — reached by `Up`/`Down` like any row, and present even with no contacts pinned yet; `Enter` on it exports rather than opening a key-details popup, and `Left`/`Right`/`d`/`o` do nothing while it's selected, since they all act on a specific contact. Either way it writes `~/.aloo/exports/<nickname>.aloo-card` and, beside it, the keybundle itself as `<nickname>.priv` (owner-only) and `<nickname>.pub` — the same two names `aloo --keygen-pq-hybrid` and aloo's own generated keys carry, so the pair can be pointed at from another machine's connect popup or `--my-key` as it is; the card is what you hand to other people, the pair is what you carry to your own other machines and never share. All under the same `~/.aloo/exports` root every other export writes under. A status notice names the three paths and the bundle's safety phrase — "exported identity card (own pqhybrid key) to `<card>` and keybundle to `<nickname>.priv` + `<nickname>.pub` - safety phrase: `<phrase>`". Purely local, never reaching the server.
...
11. **Leave a conversation with `/leave`.** Type it in the compose bar and press Enter — no argument either way; it acts on whatever is in view, and in both cases the conversation's messages go with it, since history lives in memory only. Anything worth keeping should be saved first (`autosave_messages`, item 27, or `Ctrl+E`, item 28); an existing on-disk log is never touched by this.
    - **In a channel** (the default — no private room open): targets the channel the channel selector currently names, which must actually be joined (same "leaves the typed command in place if it can't act" behavior as `/file`). Its whole tab is dropped, log included, and it comes off that selector with the membership, public or private alike; a public channel is still listed in `/channels` to rejoin from. Leaving also drops every direct peer link that channel was the only reason for (Functionality #2, `docs/PROTOCOL.md` §7.1.3). Full model in `docs/PROTOCOL.md` §6.2/§7.1.3.
    - **In a private room**: closes that conversation, drops its whole log, and takes it off the DM selector, returning to the channel view exactly as `Esc` does. Purely local — nothing is sent, nobody is told, and it is neither a block nor an unfriending: that person's next message opens the room again, empty, and puts them back on the selector.
...
14. **Live voice calls: a continuous, multi-user conversation, distinct from a voice message (Functionality #4).** Full model in `docs/PROTOCOL.md` §7.7; from the user's point of view:
    ...
    - **`Ctrl+M` mutes your own microphone from wherever you are in aloo**, without opening the call modal and finding your own row first — the same thing `m` on that row does, so the roster is told either way. **Not an OS-wide shortcut**, unlike push-to-talk (Functionality #4): it is an ordinary key of this app's, read off the terminal, so it does nothing while another window has focus — the global push-to-talk shortcut remains the only combo aloo registers with the window system. It does nothing when you're not on a call either. It only reaches aloo on a terminal that can tell `Ctrl+M` apart from `Enter` (both are `0x0D` otherwise), which is why the app asks for the Kitty keyboard protocol's disambiguation on startup; where a terminal can't, `m` on your own row still works.
...
15. **`/endotp`: ending an active one-time-pad session, in sync on both sides.** Full model in `docs/PROTOCOL.md` §16.6; from the user's point of view:
    ...
    - **Not for a pad-only pair.** Two people who hold only a one-time pad for each other and no PQ-Hybrid keys (reference table row 3, `docs/PROTOCOL.md` §16.2's `Direct` framing) have no other channel: the pad is the only way a message can reach the other side at all, so there is nothing to fall back to and nothing "ending" could mean. `/endotp` is refused for them with `OTP: /endotp does not apply to a pad-only pair - the pad is the only channel there is; delete the key from /contacts (d) to stop using it`, the session stays on, and every send keeps riding the pad. Their session is on from the moment the pad is installed on both sides and the link comes up (Functionality #17), and stops only when the key is deleted from `/contacts`.
    ...
    - **Paused stays paused until `/otp`, whoever reconnects.** The pause is written to the same per-contact store as the counters, on both sides, so the peer dropping and coming back - or your own app restarting - finds the session still ended, not silently switched back on for one side only (which is what used to happen: the side that saw the other reconnect re-derived "active" from "a pad exists", padded every send from then on, and the other side, still paused, answered in plain `PQH`). Only `/otp` - the resume handshake completing, or a pad-only pair's local turn-on - puts the pad back in use.
...
17. **Direct punching: keeping a link to someone with no server involved.** Full model in `docs/PROTOCOL.md` §7.1.5; from the user's point of view:
    ...
    - **You name who to punch at, where, and how often.** `direct_punch=on`, then one `direct_punch_to=<nickname>,<host>,<frequency>` line per person. The host may be an IPv4 address, an IPv6 address or a hostname, and may carry its own port (`bobpublic.com:19000`, `[2001:db8::1]:19000`); with none, both sides assume `direct_punch_port` (7879 by default). A port you write must be between 10000 and 65000, and one outside that is reported with the range rather than ignored. The frequency is one of `every_1m`, `every_5m`, `every_10m`, `every_15m`, `every_20m`, `every_25m`, `every_30m`, `every_35m`, `every_40m`, `every_45m`, `every_50m`, `every_55m`, `every_1h` — the `every_` prefix keeps a line unambiguous at a glance.
    - **If a fixed address never connects, name a rendezvous realm.** Most routers rewrite the port your client sends from, so a single agreed number often arrives nowhere — and which one your router picks isn't up to you, so listing more ports wouldn't help. Instead of an address, give a Hysteria Realms rendezvous both of you meet at: `direct_punch_to=bob,realm://public@realm.hy2.io/a-long-random-realm-name,every_1m`. You each learn your own router-rewritten outer address by STUN, exchange them through the realm, and punch at what you actually turned out to be. `realm.hy2.io` is the public rendezvous the Hysteria project runs; `realm+http://…` points at a self-hosted one. The realm only introduces you — no traffic goes through it — and the moment one address answers it is the only one used, losing the link starting a fresh rendezvous.
    - **One local port, bound once.** `direct_punch_port` (7879 by default) is the single UDP port you punch from and are reachable on. Binding several would not help against a router that rewrites the source port — that is exactly what the realm rendezvous discovers instead. A port already in use falls back to an ephemeral one, leaving everything except direct punching working.
...
18. **Running with no server at all (`--no-server`).** A server only ever introduces people and tracks channel membership; everything that carries content was already peer-to-peer. `aloo --daemon --no-server` (add `--foreground` to keep it in the terminal) runs with none. A plain, non-daemon `aloo --no-server` skips the connect popup entirely — there is no server to authenticate against — and either goes straight into a connected, serverless session (reachable only by the `direct_punch_to` peers in `~/.aloo/settings`) if at least one is configured, or exits immediately with a one-line explanation if none is: with nobody to reach, there is nothing for a serverless session to do.
    - **Where everything comes from.** Peers come from `direct_punch_to`, channels from `direct_punch_channel` — a comma-separated list (defaulting to `direct-punches`, so two fresh clients share a room with no setup), joined at startup, and the only channels that exist. `Ctrl+J` and `/channels` show exactly those, since there is no directory to browse and nothing to create; the Ctrl+S Direct Punch tab edits them in one comma-separated box. Your own identity is your local key; no nickname is claimed from anyone.
    ...
    - **Limits worth knowing.** You can only reach people you have talked to through a server at least once — that is where their pinned key came from — so there is no way to meet anyone new. With a fixed address at least one side needs a genuinely reachable address and port; a rendezvous realm lifts that with STUN and hole punching, though two symmetric NATs or carrier-grade NAT can still defeat it, since aloo has no relay to fall back on. A realm has a privacy cost: it reveals your outer IP to two third parties — the rendezvous (`realm.hy2.io` by default) and whichever public STUN server answers (`stun.nextcloud.com`, `stun.sip.us`, `global.stun.twilio.com` by default). Both are swappable — `realm+http://…` for a self-hosted rendezvous, `?stun=…` for your own STUN servers — and neither is trusted for anything but introducing you.
...
19. **Reconnecting to the server, for as long as the session lasts.** Full model in `docs/PROTOCOL.md` §4.2; from the user's point of view:
    ...
    - **Coming back rejoins the channels you were in**, password-protected ones included (the password is remembered for the session, never written anywhere), so other people — including people who connected while you were away — see you in the member list again. You come back with a new identity as far as the server is concerned (`docs/PROTOCOL.md` §3), which is also why everyone else's is re-learned from scratch on reconnect rather than assumed to have stayed put. Nothing of your own depends on that identity staying put: a voice message or file send in progress across the reconnect - or any made afterwards - finishes in its own row as usual, because your own rows are found by direction and stream id, never by the id the server had given you when they were made.
...
23. **Settings, and direct-punch status (`Ctrl+S`).** Once at least one `direct_punch_to` peer is configured, the header shows `<active>/<total> (next: <time>)` to the left of `Conn:`/`CPU:` — how many configured targets have a link right now, out of how many are configured, and how long until the soonest attempt — green once every configured peer is connected, yellow otherwise. Nothing is shown at all when direct punching isn't configured.

    `Ctrl+S` opens a **"Settings" popup** — the in-app editor for the client half of `~/.aloo/settings`, in four tabs. `Tab`/`BackTab` cycle the tabs (each opening on its own first field), `Up`/`Down` move between the fields of the open one, `Space` or `Enter` flips a switch, typing fills a bordered box, and `Esc` closes. The open tab is drawn filled — a background of its own the other two don't have — every switch is a filled `ON`/`OFF` block (green or red, so a tab's state reads as colour before it reads as words), and each bordered area is separated from the next by a blank row, as is the `global_ptt_shortcut` box from the switches under it. The popup is wide enough for every field's description to sit on one line, wrapping only on a terminal too narrow for it. **There is no Save button:** every change — a flip, or a single keystroke — is written to `~/.aloo/settings` the moment it is made (a merging write, so a concurrently-running daemon's or server's own keys are untouched). Each tab ends with one gray line per field naming the settings-file key and what it does.

    **Space belongs to the popup while it is open**, not to push-to-talk — which otherwise outranks every other mode and would start a recording instead of flipping the switch under the cursor.

    **Every field takes effect when you change it, not at the next start.** The sound switches and `voice_autoplay` are mirrored onto the running session, the two log switches are read per message anyway, the direct-punch scheduler is reconfigured, the No-IP updater is rebuilt and started or stopped to match, the `otp` binary is re-resolved, `queue_send_messages` switches the transport's hand-off with it, and `global_ptt_enabled` is answered per hotkey event rather than by the registration. One exception, which the field's own description says on screen: turning `global_ptt_enabled` back *on* in a run that started with it off has no OS-level shortcut to re-enable, because none was registered — that grab has to happen on a specific thread, before the runtime starts. Changing `global_ptt_shortcut` re-registers live on Linux; on macOS and Windows the owning thread is blocked inside the OS's own event pump, so there it still takes effect at the next start.
    - **General** — `voice / ptt`: `global_ptt_enabled`, `global_ptt_shortcut`, `transfers_shortcut` (the in-app chord that opens the transfers popup — `ctrl+d`, `alt+t`, `f5`; a bare letter is refused; one that names no key leaves the stored one alone), `touch_ptt_enabled`, `voice_autoplay`, `roger_beep`. `notifications`: `sound_notifications`. `logs`: `autosave_messages` (item 27), `resume_from_log` (item 29). `delivery`: `queue_send_messages` (item 34). See item 32 for what the three sound switches silence.
    - **Direct Punch** — the `direct_punch` master switch over the list of configured targets, and the `noip` area (`noip_when_no_server_and_direct_punch_is_active` plus `noip_hostname`/`noip_username`/`noip_password`; the password is drawn as dots unless it is the box being typed into; unlike the punch schedule, the No-IP updater is resolved once per session, so those four take effect on the next start). In the target list, `a` adds one, `Enter`/`e` edits the selected one prefilled, `d` deletes it, and `Up`/`Down` walk its rows while there is another row to move onto — at either end the same keys carry on to the next field, so one set of arrows drives both the field column and the list inside it. Each field of the add/edit form is its own titled, bordered box — the same look the connect popup's own fields use — with the terminal's own cursor blinking in the nickname box the instant the form opens (the default first focus) and tracking whichever free-text field (nickname/host/port) `Tab` moves to next; frequency is a bounded `Left`/`Right` selector over the same values `~/.aloo/settings` accepts, so it has no cursor of its own. Saving persists the whole list and reconfigures the scheduler immediately — the new schedule is live the same tick, not on the next restart — and shows a confirmation notice. Flipping the master switch is live the same way: off stops the punching at once, on restores exactly the listed targets.
    - **OTP** — `otp_low_key_warn_pct` (digits only, at most three) and `otp_binary_path` (empty means "find `otp` on `PATH`"). A percentage that is not one — an empty box mid-edit, or a number outside 1–100 — leaves the stored value alone rather than writing a nonsense one.
    - **File Sharing** — `file_sharing_link_speed_kbps` (digits, up to eight) and `file_sharing_max_pct` (digits, at most three, 1–100; an unusable value leaves the stored one alone), applied to running shared transfers the moment they are typed, and the list of shared folders — `a` adds (folder path, then `all` or comma-separated nicknames), `Enter`/`e` edits, `d` deletes — saved and re-announced on every change (Functionality #35).

    Everything a server reads (`server_*`), everything a daemon is configured with (`daemon_*`), and the connect cache (`connect_*`) stay hand-edited: the first two belong to a process this popup is not part of, and the third is written *by* connecting rather than typed.
...
27. **Autosaving chat/voice history (`autosave_messages`).** Off by default; set `autosave_messages=on` in `~/.aloo/settings` to have every channel/DM message — text, voice, file-transfer notices, presence lines, everything that appears in the scrollback — appended, as it happens, to a plain-text log under `~/.aloo/exports/<server>/{channels,dms}/<name>.log`, `<server>` being the host and port last connected to (or the literal `DIRECT` for a `--no-server` session). Each line is timestamped in UTC — `[2026-08-26T14:23:01Z] <- alice: hello`, the arrow showing direction. A finished voice message — one you sent as much as one you received; yours is written the moment your recording ends and its row becomes a finished voice message — additionally gets a `<utc>_<nickname>.wav` file written beside its `.log`, referenced from the line by filename. A voice message still streaming when the reader of item 29 opens is not on disk yet, and is not counted as if it were. Existing logs are only ever appended to, never replaced or truncated, across restarts and separate sessions. Read once at startup — turning it on by hand in the file takes effect on the next run; the `Ctrl+S` popup (item 23) is the way to change it without restarting.
...
31. **Clicking with a mouse, where available.** Mouse capture is on for the whole session; a left click on the input bar focuses it, and a left click on a row in the member sidebar (while actually viewing a channel, not a DM) selects that member and focuses the sidebar — the same click-to-focus a chat app is expected to have. A click does nothing while a popup or any other overlay is in front of the view (nothing to click through to), and does nothing while viewing a DM (the sidebar isn't shown there at all). A left button held past a quarter of a second is hold-to-talk (Functionality #4), so its release is handled too - and a press and release arriving together, the finished tap a touch terminal sends in place of a press it never forwards, is what tells aloo hold-to-talk cannot work in that terminal (it says so once); right clicks, drags and scrolling aren't yet. Most terminal emulators still let a held modifier key (commonly Shift) bypass this for native text selection/copy.

32. **Turning the sounds off, one at a time (`voice_autoplay`, `roger_beep`, `sound_notifications`).** All three on by default, each silencing exactly one thing and saying nothing about the other two; all three live, changeable from the `Ctrl+S` popup (item 23) without a restart.
    - **`voice_autoplay`** off keeps *every* arriving voice message off the speakers — whoever sent it, and whether it came over pq_hybrid or under an OTP pad. The blanket form of `/mute-voice` (item 16), which silences named people only; both are asked through the one predicate, so nothing can honour a mute and forget this. The row is still logged, still marked unlistened, and `Enter` still replays it on demand.
    - **`roger_beep`** off silences `assets/end.wav`, the "over" tone, at both ends of a voice message — when the sender releases Space and when an arriving one finishes.
    - **`sound_notifications`** off silences every *event* sound: `assets/bell.wav` (an incoming file offer, an OTP prompt, an identity to review), `assets/joined.wav` (a daemon's focused contact appearing), and `assets/ping.wav` (being `@`-mentioned, item 33). `end.wav` is deliberately not one of them — it punctuates speech rather than announcing an event, and `roger_beep` owns it.

33. **Being named with an `@`.** A text message that writes `@<your nickname>` — in a shared channel or in a DM, over pq_hybrid or under an OTP pad alike — plays `assets/ping.wav` once as it arrives, alongside being logged as usual. A mention is a whole word at both ends: the `@` may not follow a nickname character (so an address like `bob@me.example` is not one) and the nickname may not be followed by one (so `@meredith` is not `@me`). It is **case-sensitive**, because the server tells `bob` and `Bob` apart and a case-folded match would sound one person's ping for a message addressed to another. Only a text message can name anyone — a voice or file row never does — and your own sends never take the receive path, so writing your own name cannot ping you. Silenced with `sound_notifications` (item 32).

34. **Holding a message for someone who isn't there (`queue_send_messages`).** On by default. The transport already holds content for a minute while a link blinks (`docs/PROTOCOL.md` §7.1.1's pending queue); this is what covers the other person simply not being there — for an hour, a day, or across a restart of aloo. What you send them is written to `~/.aloo/outbox/<nickname>.q` and delivered the moment their link opens, **in the order you sent it** — including across a restart of aloo, since what is on disk is read back at the next session start and goes out as each recipient becomes reachable.
    - **Text and voice, not files.** A file transfer is a live, consent-gated, chunked conversation with the receiver (§9) — an offer they must accept before a single chunk is sent — so replaying half of one an hour later isn't a delivery, and it fails the way it always did. Neither is anything that states something about *right now*: delivery receipts, acks, pad-provisioning chunks and call control are let go rather than delivered as a lie later. Those still wait in the transport's own short queue exactly as before, since "not worth keeping for an hour" isn't "not worth keeping for the minute this link is being punched".
    - **Under a one-time pad, sealing is spending, so the rules are stricter.** A message written into an active `/otp` session is encrypted the moment you write it — its pad position gone there and then, reachable peer or not — and the sealed ciphertext goes into its own durable queue (`~/.aloo/otp_outbox/<contact>.q`, keyed by contact name so it outlives a reconnect). Delivery is **one message per acknowledgement, in the order they were sealed**: the front of the queue is the only one that may be on the wire, and it stays there until its own proof-carrying ack comes back, so a send that left but was never answered is retried rather than skipped. Those entries are pad output, so they are zeroized as they leave memory and their file is overwritten before it is unlinked. A pad-wrapped **voice message** is held the same way, in the same queue: it is sealed when you finish recording rather than when they accept it, so nothing readable sits on disk while they are away — the offer that announces it takes one pad position and joins the queue, and the recording takes the next and joins right behind it, as its own entry pointing at its own ciphertext file (a recording can be megabytes, so the queue keeps a reference to the file `otp --encrypt` produced rather than a second copy; the file lives in the queue's own directory and is securely erased when the entry is delivered or swept). Because a recording is far larger than a line of text, the remaining key is checked before any of it is spent and the message is refused outright if it will not fit. Ordering needs no special rule: written order is queue order, the recording included, so anything you write afterwards waits behind it and their pad is never read out of turn. The one thing that is never sealed ahead is a message behind a spend the queue does *not* hold - a file offer, a file's content, the end notice - since those are retried from the keychain's single `.last_sent` copy, which the next seal would overwrite: a text typed then waits as plaintext (and says so, "queued") until that spend is acknowledged, and a voice message is refused with nothing spent. Turn queueing off and voice goes back to the older shape: staged unencrypted, encrypted on accept, and refused while any previous send is unacknowledged.
    - **It keeps the encryption you sent it with.** What's stored is the already-sealed payload, byte for byte — `pq_hybrid`, or the one-time pad if an `/otp` session was open with that peer when you wrote it. It's never re-sealed on the way out, which is what keeps plaintext off the disk; a voice message's chunks are sealed before they reach the transport for the same reason. Two consequences, both deliberate: a pad-wrapped message spends its pad position when it's *queued*, which is why the order is load-bearing; and a `pq_hybrid` message is sealed against the recipient's key as it was then, so a peer who rotates their identity while away can't open what was held for them — it fails visibly, rather than being silently re-sealed behind your back.
    - **A voice message can be recorded for someone who isn't reachable yet.** Normally a recording simply leaves out anyone whose link isn't already `Active` (§11.2). With this on they stay in the stream instead, their chunks sealed like everyone else's, waiting on disk.
    - **`i` on a held message says `QUEUED`**, in the same partway-there colour a channel send wears while only some of its recipients have it — a stronger claim than `UNDELIVERED`, which says only that nothing has come back yet. It reverts to `UNDELIVERED` the moment the queue is flushed and that leg is genuinely on the wire again, and says nothing once the recipient has it: `DELIVERED` and what it grows into always win.
    - **Nothing ages out, and nothing is evicted.** A held message waits as long as it has to — how long it has been there says nothing about whether it should still go. The **only** thing that ever removes one is this machine no longer holding key material for the contact it was sealed for: once they're deleted from `/contacts` (which takes every device's identity pin and each one's OTP keychain entries with it), nothing queued for them could be delivered *or* read back, so it goes and their queue file with it. Swept once at session start and every 12 hours after, for both queues.
    - **With it off, nothing is held for someone who isn't there, and no pad is spent on them.** A pad message to someone unreachable is refused *before* `otp --encrypt` runs and its row marked failed — a spent pad position with nowhere to go would be worse than a refusal you can see. What the switch does **not** govern is where a message waits its *turn*: a pad send is one-at-a-time, so anything written while a previous one is still unacknowledged waits in the same durable queue in both positions of the switch — sealed, on disk, and shown as `QUEUED`. Waiting is precisely when a crash would otherwise lose it, and the alternative was holding it in memory as plaintext, which lost it silently on a restart and left it unencrypted in the meantime.
    - **A new message always joins the back of the queue**, never goes out ahead of it — including once their link is up again, right until the queue is actually drained. Overtaking would deliver a message before ones sealed earlier, which for a pad-wrapped run isn't a reordering but a break: the receiver's pad expects exactly the sequence it was given.
    - **A spent pad position always has a message behind it.** Sealing spends, and a spend can't be taken back, so once `otp --encrypt` has run the sealed bytes have to end up somewhere that will deliver them. The queue therefore *reports* whether it took the message, and decides every refusal before it changes anything; one it won't take goes straight at the transport instead of being dropped. (A disk write that fails after it's already in memory still counts as taken — it will go out, it just wouldn't survive a restart; sending it directly as well would put one pad position on the wire twice.) The one refusal that's safe is the one that happens *before* any spend: with queueing off, a message to someone unreachable is refused before `otp --encrypt` runs at all. Turning queueing off doesn't abandon what's already sealed and waiting either — those positions are spent, so the pad queue is kept and allowed to drain, and only goes once it's empty. This is what keeps the two ends' pads in step no matter how a send goes wrong.
    - **A held message is deleted when *they* have it, not when it's sent.** Handing something to the transport isn't delivering it — the link can die mid-flight, and aloo can be closed, in either case with the message neither delivered nor still queued. So the drain leaves the message on disk and only the recipient's acknowledgement removes it. Anything still there when a link comes up goes out again, which means a message can occasionally be sent twice; the receiving side recognises the repeat and shows it once. (Voice is the exception: it's unacknowledged by design, so a held voice chunk is released as it goes.)
    - **Once it arrives, it is shown.** A message the recipient's client accepted is never quietly lost on the way to the screen. Two things could have cost one, and neither does. A held message keeps the sequence number it was sealed with, so it lands *after* newer ones once the peer is back — the receiver's replay protection is a window over recent sequence numbers rather than a "must be the newest" rule (`docs/PROTOCOL.md` §13.4), so late is not read as an attack. And a punched link can carry a message before the server has said who the sender is, leaving nothing to decrypt it with and no name to put on it — that one is held and re-tried every turn of the loop until they are known, rather than dropped for arriving early. Where it then appears follows the ordinary rules: a DM you closed with `/leave` reopens carrying it, and a sender still under identity review has it held and revealed when you Accept them (§12) — so opening the conversation always shows what came in while you weren't looking at it.
    - **Keyed by nickname**, which is what survives a reconnect (a `UserId` never does, §3) — the same choice, and the same caveat, as `/mute-voice`. Turning it off hands the transport back its own in-memory queue (which keeps its order the same way); whatever is already on disk is kept, not discarded, and goes out if you turn it back on.
35. **Shared folders: standing access to folders of yours, for the people you name.** Full model in `docs/PROTOCOL.md` §7.8; from the user's point of view:
    - **Sharing.** One `share=<path>,all` or `share=<path>,<nickname>,<nickname>...` line per folder in `~/.aloo/settings`, or the `Ctrl+S` **File Sharing** tab (item 23), whose list works exactly like the Direct Punch tab's: `a` adds, `Enter`/`e` edits, `d` deletes, and every change is saved and announced to everyone currently linked, on the spot. A folder is known to others by its last path component only (`Photos`, never where it lives), so two shares with the same name are refused. Access is by nickname — the same identity a pin names — so a grant survives reconnects and can name someone not yet met. A malformed line is skipped and logged, like a bad `direct_punch_to`.
    - **Being told.** Whoever you share with learns of it the moment your link to them is up (and again whenever the list changes — taking a folder away removes it on their side too). Opening a DM with someone who has shared something with you prints one gray system line in that room, `<nickname> has given you access to files, type /info to access` — once, and again only after they share anew; if the room is already open when the access arrives, the line is printed then.
    - **Browsing.** `/info` in that DM (or `i` on them in the sidebar) shows a **Browse shared files** button under their keys — present only for someone who shares something with you; `Enter` presses it. The browser popup opens on their share names; `Enter` opens a folder (asking them for its listing, shown as `loading…` until it lands), `Backspace`/`Left` goes up (and back to the share list from a root), `Esc` closes. Each row shows `<folder or file name> <created> <updated> <size>` — times in local `YYYY-MM-DD HH:MM`, `-` where the filesystem records none; sizes in `B`/`KB`/`MB`/`GB`. A folder with more than 500 entries shows the first 500 and says so.
    - **Downloading.** `d` on a file asks for that file; `d` on a folder (a share name included) asks for every file under it. There is **no Accept/Reject popup** — you asked for it — and **nothing is written into the conversation**: a download is something you fetched, so it lives on the Downloads tab instead. Files land under `~/.aloo/downloads/fileshare/<their nickname>/<share>/<path inside the share>`, keeping the owner's own layout, so a folder called `test` arrives as a folder called `test`. While a file is arriving it is written under its final name plus `.part` and becomes itself only once whole, so nothing half-downloaded ever looks finished. Several files come at once (four), the next starting as each finishes. Only what you asked for skips the popup: an offer nobody asked for, from anyone, still gets one, and an offer from someone still under identity review is held exactly as before.
    - **The Downloads tab.** `Tab` in the browser switches between the folders and everything you have pulled from that person; while anything is running the other tab's name reads `Downloading <n>...`. Each row shows what was asked for, who from, how far along (the same progress bar a one-time pad transfer draws), and how many files of how many — with anything still going listed above everything finished with. `c` cancels a running one, `r` resumes a stopped one, `x` removes a finished row, and `X` clears every finished row at once. Cancelling never deletes what already arrived: only the part-written file goes. Resuming asks for the same folder again and skips every file already on disk at full size, so only what is missing moves.
    - **While anything is moving**, the header shows a blinking yellow arrow and the speed in kilobits per second — `↓` for what is coming in, `↑` for what is going out of your shared folders, both at once if you are doing both — ahead of the direct-punch and connection indicators.
    - **Everything, everywhere: `Ctrl+D`.** The Downloads tab shows one person's; this popup shows every file-share transfer this client has taken part in, in both directions and with everyone — what you are pulling from others, and what others are pulling from your shared folders, each row saying which way (`↓`/`↑`) and with whom. The three views — everything, downloads, uploads — are tabs across the top, each showing how many are running under it, and `Tab` moves between them. A long path is shown by its end with a leading `…`, since the last components are what tell one row from another. `c` stops the selected transfer whichever side you are on: on a download it tells the owner to stop offering, on an upload it tells the person pulling to stop waiting. `r` resumes a stopped download — the same row on both screens, carrying on rather than a second item — `x` removes a finished row, `X` clears every finished row. A running transfer cannot be removed without cancelling it first. **The history survives a restart** — it is kept in `<client home>/transfers` — and anything that was still running when the app closed comes back marked as interrupted, since nothing resumes on its own. The key is `transfers_shortcut` in `~/.aloo/settings`, editable on the `Ctrl+S` **General** tab, and takes effect the moment it is changed.
    - **Under OTP.** With a live pad session to the owner, each shared file goes through the pad like any other file — two spends per file, one at a time.
    - **The owner's budget.** `file_sharing_link_speed_kbps` (your upload speed; `0`, the default, means unknown and no cap) and `file_sharing_max_pct` (default 50) — both on the File Sharing tab, both applied live, even to transfers already running — cap every shared send together at that fraction of that speed. An ordinary `/file` send is never capped.
    - **Writing the path.** Absolute (`/srv/Public`, `C:\Users\me\Public`, `\\server\share\Public`) or starting `~/`, which is expanded from `HOME` — or `USERPROFILE` on Windows. A shell variable (`$HOME`, `${HOME}`, `%USERPROFILE%`) is **not** expanded, and a relative path would depend on wherever aloo was started; both are refused in the `Ctrl+S` form with the reason, rather than becoming a folder nobody can open. A share whose folder is missing or unreadable is reported to **you** — in the status line at start-up, and again whenever someone's request fails on it — because the person browsing it only learns that it is not readable on your machine, which they cannot fix.
    - **Revoking works mid-download.** Access is checked again for every file as it is sent, not once when the folder was walked — so taking someone off a share's list, deleting the share, or sharing a subfolder more narrowly stops everything still queued for them, not just their next request.
    - **Overlapping folders.** If one shared folder sits inside another, the **most restrictive** of the two applies: someone who may see `/work` but not `/work/payroll` does not see `payroll` listed inside it, cannot name it directly, and does not receive it when downloading `/work`. The same holds for one folder shared twice with different lists.
    - **Across platforms.** A folder is known by its own final path component, read with the sharer's own separator rules, so a Linux, macOS or Windows folder keeps the name its filesystem gives it. Names that one platform allows and another does not are handled on arrival rather than refused at the source: a folder called `CON`, or a file with `<`, `>` or `|` in its name, is saved under a name Windows can create too (the same sanitising an ordinary file transfer already does), and a file whose name contains `\` or `:` is served normally from Linux and macOS while Windows, where those mean something, refuses them.
    - **Links follow where they point, not where they sit.** A shortcut or symlink inside a shared folder is followed only while it lands *inside* that folder. One pointing at a file outside, at a folder outside (or anything under it), or into a subfolder the person isn't allowed to see is neither listed nor downloadable — not shown and then refused, but absent, so the listing itself can't be used to find out what's outside. A link that stays inside the shared folder works like the ordinary file it names.
    - **Safety.** Access is checked on every request, never trusted from the announce; a path is confined to the share (no `..`, and a symlink out of the share is refused); a peer sees nothing about a folder it was not granted, not even that it exists — asking for one answers exactly as asking for a folder that does not exist. Someone whose identity is still under review (Functionality #8) is served nothing and told nothing until you have accepted them.
...
**At least one of you must be reachable, or you both name a realm.** With a
fixed address and no rendezvous, there is no STUN and no relay, so the
address in the other's settings has to actually arrive - a forwarded UDP
port, or a host with a public address; both behind NAT with nothing
forwarded will never open that way. A `realm://` rendezvous is what lifts
that: both of you meet at the same realm, discover your outer addresses by
STUN, and hole-punch. It works for many NATs but not two symmetric ones,
and never behind carrier-grade NAT, where nothing but a relay (which aloo
does not provide) would.
...
| Serverless direct punch (§7.1.5) | `settings.rs` `DirectPunchTarget`, `DirectPunchVia`, `PunchFrequency`, `DEFAULT_DIRECT_PUNCH_PORT`, `PUNCH_FREQUENCIES`; `client/p2p.rs` `configure_direct_punch`, `direct_tick`, `on_direct_ping`, `on_direct_pong`, `direct_peer_id`, `is_direct_peer_id`, `utc_second_of_hour`, `DIRECT_PUNCH_WINDOW`, `DIRECT_MAX_RECONNECTS`; `p2p_proto.rs` `MAX_DIRECT_PUNCH_NICK_LEN` |
| Rendezvous realm, STUN and punch (§7.1.5) | `client/hysteria_realm.rs` `RealmAddr`, `PunchMeta`, `encode_punch`, `decode_punch`, `stun_binding_request`, `parse_stun_binding_response`, `RawTaps`, `punch_candidates`, `run_attempt`, `role_for`; `client/p2p.rs` `spawn_realm_attempt`, `on_realm_outcome` |
...
| Shared folders (§7.8) | `settings.rs` `SharedFolder`, `ShareAccess`; `client/shared_folders.rs` `resolve_shared_path`, `list_directory`, `collect_download`, `SharePacer`, `download_dest`; `client/session/shared.rs` `send_shared_folders`, `pump_shared_sends`, `tag_incoming_offer`, `drain_auto_accepts`; `proto.rs` `Content::SharedFolders`; `p2p_proto.rs` `P2pPayload::SharedFileTag` |
...
| `/endotp`: local pause, notice, and the mail/multi-session guards (§16.6) | `client/otp.rs` `handle_end_otp_command`, `decide_end_otp`, `EndOtpDecision`, `on_end_session`, `on_end_session_ack`, `send_end_notice_now`, `end_notice_may_go_now`, `pause_session_locally`, `recover_and_resend_envelope`, `send_sealed_end_session_ack`, `contact_name_if_session_live`; `client/otp_store.rs` `OtpStore::mark_end_requested`, `pause_after_peer_ended`, `resume_session`, `is_paused`, `OtpContactState::paused`, `PendingOtpContent::EndNotice`; `client/session/link_events.rs` `retry_owed_otp_exchanges`; `client/tui/ui.rs` `UiAction::EndOtpSession`, `submit_input` (`/endotp`); `proto.rs` `Content::OtpEndSession`/`OtpEndSessionAck` |
...
| A session surviving a reconnect until `/endotp` - and a confirmed end surviving a reconnect until `/otp` (§16.6) | `client/otp.rs` `contact_name_if_active`, `contact_name_if_session_live`; `client/otp_store.rs` `OtpContactState::paused`; `client/session/server_events.rs` `handle_server_message`'s `UserJoined` arm and `client/session/identity.rs` `maybe_resolve_p2p_identity_data`, `register_pad_only_peer` (re-mark a reconnected peer active only for a provisioned session that is not paused) |
...
| Shared-folder download: tag, then the same offer — accepted with no popup | `session/shared.rs` `pump_shared_sends` (owner), `tag_incoming_offer` + `drain_auto_accepts` (requester), `docs/PROTOCOL.md` §7.8 |
| Shared sends paced to the declared link speed | `shared_folders.rs` `SharePacer`, debited per chunk in `spawn_send_file_worker` |
...
| `AuthCheck::RegisteredElsewhere { server_addr }` | federation only (below): no local account, but the federation directory says a peer owns this nickname — refused, naming that server, before the (slow) credential check ever runs |
| `AuthCheck::Conflicted` | federation only: this nickname is claimed by more than one federated server — refused for any new login until an operator resolves it |
...
Two or more servers, each configured with `server_federation_*` settings naming the others as peers (`docs/PROTOCOL.md` §18), can share one nickname/channel namespace instead of each being its own island. A nickname and a channel name are each meant to belong to exactly one federated server: `Register` and a new channel's creation both check every linked peer's federation directory first, refusing a name already owned elsewhere by naming which server owns it, and a login for a nickname registered on a different federated server is refused the same way rather than with the ordinary undifferentiated "wrong password or no such account" answer - a deliberate, narrow exception to that anti-enumeration property, since the whole point is telling the user which server to use. Two servers that independently claim the same name (federating two already-populated servers whose namespaces overlapped, or an unresolved race) become `Conflicted` rather than one silently winning: refused for any *new* login/registration/creation until an operator resolves it by hand, but no already-connected session is ever dropped and no account is ever removed automatically.

The federation link itself is server-to-server, over its own port, secured by a dedicated PQ-hybrid handshake with each side's durable identity pinned individually per peer by public key - no TLS or certificates - a second, independent trust domain from the client-facing `server_ssl`, which authenticates only the server to an ordinary client. It carries only nickname/channel ownership metadata (and, for the two features below, opaque OTP mail ciphertext and channel-join verdicts) between servers, never message content itself, matching the single-server doctrine above.

Joining a channel homed on a different federated server works by proxy: the request goes to the owning server, which is the only one that ever checks the password/ban/allowlist, and the requesting server mirrors a successful grant locally so its own client sees an ordinary `Joined`. A public channel created anywhere in the federation is also mirrored (empty of members) into every linked peer's own channel registry the moment it is gossiped, and announced live to already-connected clients there, so the whole federation shares one public channel list rather than each server only learning of a channel once a local client happens to join it by name; a private channel stays reachable only by knowing its name, exactly like a local one. The channel's home server further gossips every join or leave on it - local or proxied - to every linked peer, so members connected to *different* federated servers see each other's `UserJoined`/`UserLeft` even with no direct link between those two servers at all, relayed purely through the shared home. OTP mail addressed to a nickname registered on a different federated server is forwarded there and delivered exactly as it would be locally, with delivery receipts relayed back the same way - the uploading server keeps its own durable copy until the peer confirms receipt, so a briefly-unreachable link delays delivery rather than losing it. Two clients connected to *different* federated servers can also talk to each other: the candidate exchange every direct link starts with (§7.1) is relayed between servers - straight across when they are linked, and otherwise through a server they share, so a hub topology behaves like a full mesh - after which the conversation is an ordinary direct link that no server sees. Relaying is allowed only between two people who actually share a channel, checked by both servers rather than taken on trust, since a candidate address is a client's real IP. See `docs/PROTOCOL.md` §18.5.

Deleting a channel or removing an account gossips that removal too, so a name a server no longer has stops being permanently blocked federation-wide - a peer mirroring a deleted channel force-deletes its own copy and tells its local members, the same as a local deletion would (`docs/PROTOCOL.md` §18.8). Every federation event a server takes part in - a peer link coming up or down, directory gossip sent or received, a channel or nickname's removal, a channel member joining or leaving, a channel join proxied to or from a peer, and OTP mail forwarded, received, or confirmed delivered - prints one line to the server's own console (`Federated server <peer> - <EVENT>`), always just an event name and which peer it concerns, never a nickname, channel name, or any message content.
