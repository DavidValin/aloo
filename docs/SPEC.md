...
- as the glyph alone, prefixing **every message in that room the pad protects**, and at the **start of the compose bar** those messages are typed into — the bar says what will happen to the next message, not only what happened to the last.
...
A text message never reaches a `+` state, having nothing further to do with it, and the log's own arrow is unmoved by any of this: it stays a three-state summary of who has the message, not of what they did with it.
...
8. **Identity pinning (`id_store`): deciding whether to trust a nickname that reconnects under a different key.** Full model in `docs/PROTOCOL.md` §12; from the user's point of view:
   ...
   - **`/contacts` manages every pinned nickname's three keys: PQH, OTP, OTP MAIL.** Each row shows all three as a green ✅ or red ❌ badge naming whether it exists. `Left`/`Right` cycles which one is highlighted — across the *whole list* at once, so paging through contacts with `Up`/`Down` keeps comparing the same key — and `Enter` opens that row's key details popup. The popup always shows a fixed, yellow explanation of what that key is for (PQH pins an identity so you can use pqh encryption at all; OTP is for live sessions when you're both online; OTP MAIL is for mail that waits, encrypted, until the recipient comes online — `docs/PROTOCOL.md` §16.1.1/§17), then either its path on disk (plus, for OTP/OTP MAIL, the same seq/offset/remaining-per-direction figures shown elsewhere) and a **Delete key** action behind a confirm, or — if it doesn't exist — **Create key** (PQH: import a self-signed identity card via a file browser, `docs/PROTOCOL.md` §12.6) or **Install manually** (OTP/OTP MAIL: the same two-file-path form `o` already opens, just filing the result under whichever key was selected). `Left`/`Right` inside the popup switches which key it's showing. Deleting the PQH key forgets the whole contact — the identity pin and both other keys, since they can no longer be named without it; deleting OTP or OTP MAIL alone touches only that one. Every one of these actions **takes effect immediately**: the list re-gathers its rows, and an already-open `/mail` compose view for that same nickname has its recipient check re-run without retyping anything — unblocking the hard gate above the instant a mail key appears, and re-blocking it the instant one is removed.
...
15. **`/endotp`: ending an active one-time-pad session.** Full model in `docs/PROTOCOL.md` §16.6; from the user's point of view:
    - **Either side may end a session alone**, no accept/reject needed the way starting one does. Typed in the same open DM room `/otp` was started from, it destroys this side's own copy of the pad immediately and shows "OTP session ended" the same way starting one shows "OTP session started" (§16.3). Refused, with a status notice, if there is no active session with that peer, or if an OTP mail (Functionality #13) to that same contact is still awaiting delivery confirmation.
    - **The one command still usable while its peer is offline.** Every other DM action is refused outright while the compose bar's peer is offline (Functionality #7's "offline peer's room refuses to send"), but `/endotp` is the deliberate exception: it works regardless, and the compose bar shows what's actually typed the moment anything is, rather than the fixed "(user offline)" notice.
...
23. **Direct-punch status and editing (`Ctrl+S`).** Once at least one `direct_punch_to` peer is configured, the header shows `<active>/<total> direct punches, next try in <time> (Control+s)` to the left of `Conn:`/`CPU:` — green once every configured peer is connected, yellow otherwise. Nothing is shown at all when direct punching isn't configured. `Ctrl+S` opens a "Direct Punches" popup listing every configured target: `a` adds one, `Enter`/`e` edits the selected one prefilled, `d` deletes it, `Esc` backs out one level at a time. The add/edit form's nickname/host/port fields are free text; its frequency is a bounded `Left`/`Right` selector over the same values `~/.aloo/settings` accepts. Saving persists the whole list back to `~/.aloo/settings` (a merging write, so a concurrently-running daemon's own keys are untouched) and reconfigures the scheduler immediately — the new schedule is live the same tick, not on the next restart.
...
| Origin/order verification refusing a message before any key is spent | `client/otp_cli.rs` `OtpCliOutcome::Rejected`, `FileCliOutcome::Rejected`; `client/otp.rs` `UnwrapOutcome`, `unwrap_incoming`, `unwrap_or_notify`, `finish_incoming_file`; `client/otp_mail.rs` `on_mail_deliver` |
...
| `/endotp`: local pause, notice, and the mail/multi-session guards (§16.6) | `client/otp.rs` `handle_end_otp_command`, `decide_end_otp`, `EndOtpDecision`, `on_end_session`, `on_end_session_ack`, `send_end_session_payload`, `queue_end_session_payload`; `client/otp_store.rs` `OtpStore::pause_session`, `pause_after_peer_ended`; `client/tui/ui.rs` `UiAction::EndOtpSession`, `submit_input` (`/endotp`); `proto.rs` `Content::OtpEndSession`/`OtpEndSessionAck` |
