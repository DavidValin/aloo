...
* Status ⚠️ BETA (under heavy development and testing)
...
- **A server it can't reach yet is waited for, not given up on.** Started at login before the wifi is up, or with the wifi off, the daemon keeps trying — right away, then 5s, 10s, 20s and every 30s, for as long as it takes — and connects the moment the server is reachable, with nothing restarted. `aloo --daemon-status` says it's waiting and why, a bare `aloo` attaches and shows the countdown, and `aloo --daemon-stop` ends the wait cleanly. Only an actual answer from the server — a wrong password, a taken nickname — is still a failed start.
...
**If a fixed address never connects, meet through a rendezvous.** Most routers change the port your client sends from, so a single agreed number often arrives nowhere — and which one your router picks isn't up to you, so listing more ports wouldn't help either. Instead of an address, name a **rendezvous realm** you both meet at — a public one the Hysteria project runs (`realm.hy2.io`):

```
direct_punch_to=bob,realm://public@realm.hy2.io/a-long-random-realm-name,every_1m
```

Agree on the same long, unguessable realm name and both add a line like this. Each of you learns your own router-rewritten outer address from a public STUN server, swaps it through the realm, and then punches straight at where the other actually turned out to be. The realm only introduces you — nothing you send goes through it. For a self-hosted rendezvous (`hysteria-realm-server`), `realm+http://token@host:port/name` points at it instead.

Your local port stays a single one — `direct_punch_port` (7879 by default) — since a router that rewrites the source port makes binding several pointless; the rendezvous discovers the outer address rather than guessing it. The moment one address answers, that's the only one used from then on; losing the link starts a fresh rendezvous.
...
Channels come from your settings (`direct_punch_channel=general,dev` — comma-separated, and defaulting to `direct-punches` so you and anyone you punch share one room out of the box) — those are the only ones that exist, and they're what `Ctrl+J` and `/channels` show. You don't even need a shared channel to talk: two peers who punch each other are reachable as a DM regardless; a shared channel name just also puts you in a room together. Everything that actually carries anything still works: messages, voice, push-to-talk, files, calls, and live `/otp` sessions are all peer-to-peer and never needed a server.
