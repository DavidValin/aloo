...
- 🔇 **Mute one person's voice** — `/mute-voice <nickname>` stops that person's voice messages playing themselves the moment they land; `/unmute-voice <nickname>` undoes it, and either command with no nickname lists who is muted. It isn't a block: their messages still arrive and still show in the log, so `Enter` replays them whenever you want, and they're marked 🔇 in the sidebar so you can see why the channel went quiet. Kept in `~/.aloo/settings`, so it survives reconnecting — you can even mute someone before they've ever connected. Live calls are unaffected.
- 📞 **Live voice calls** — type `/call` in a channel or DM to start a continuous, multi-user call, distinct from a walkie-talkie clip: not push-to-talk, no time limit. You confirm first, told how many people it will ring; everyone reachable then gets an Accept/Reject popup naming you, and everyone who accepts hears everyone else, still peer-to-peer, no server relay. A call modal shows the live duration, who's on it (the host named `<nickname> (host)`, then `IN CALL`, `INVITED`, `REJECTED`, `MUTED`) and a moving voice bar per person — `Esc` folds it away into the top row's red-bordered `🔴 Call` box, `Ctrl+R` brings it back. `m` on your own row mutes your mic — yours alone to lift, though everyone's roster shows it; as host, `m` on someone else's row mutes them (only you can lift that) and `i` invites one more person. Leaving ends the call for everyone, and `/endcall` leaves it. One call at a time, and not available over an `/otp` session.
...
- 📢 **Public channels** — join the channels the server advertises, picked from the channel selector across the top; a channel created after you connect shows up live, no reconnect needed.
...
- Type `/leave` and press **Enter** to leave the selected channel — it drops off the channel selector, public or private; a public one stays in `/channels` to rejoin from.
