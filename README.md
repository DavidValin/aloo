...
- 📞 **Live voice calls** — type `/call` in a channel or DM to start a continuous, multi-user call, distinct from a walkie-talkie clip: not push-to-talk, no time limit. Everyone reachable gets an Accept/Reject popup naming you; everyone who accepts hears everyone else, still peer-to-peer, no server relay. A permanent red banner marks the whole time you're on one. `/mute` toggles your own mic without ending the call, `/endcall` leaves it. Not available over an `/otp` session.
