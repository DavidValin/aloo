...
Feature: Showing how each person's identity is protected
  ...
  convention across all five my_key types - reading "who this is, then how
  ...
  Scenario: The sidebar tags each person according to how they hold their key
    ...
    And bob is in the channel with me using rsa
    And carol is in the channel with me using rsa_per_msg
    ...
    Then bob's tag is shown after their name
    And carol's tag is shown after their name
    And dan's tag is shown after their name
  ...
  Scenario: A private room titles itself with the peer's tag too
    Given I am connected and viewing a channel
    And bob is in the channel with me using rsa_per_msg
    When I open a private room with bob
    Then the private room title reads "Private: bob" with the rsa_per_msg tag after the name
